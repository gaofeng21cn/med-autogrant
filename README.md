...
- To inspect the OPL Foundry Agent series command surface, use `uv run mag foundry status --json` or `<med-autogrant-repo>/scripts/run-python-clean.sh -m med_autogrant.cli foundry status --format json`; `mag` is equivalent to `medautogrant`. The help first screen identifies `Series: OPL Foundry Agent`, `Agent id: medautogrant`, the ordinary path `workspace/work/stage/run/vault/handoff/connect`, the executable command surface, and the MAG authority boundary. `status`, `inspect`, `interfaces`, `validate`, `doctor`, and `peers` are top-level shortcuts, while `grant` aliases `workspace` and `work` aliases `pass`.
