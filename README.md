...
- To inspect the OPL Foundry Agent series frontdoor, use `<med-autogrant-repo>/scripts/run-python-clean.sh -m med_autogrant.cli foundry status --format json`; `status`, `inspect`, `interfaces`, `validate`, `doctor`, and `peers` are top-level shortcuts, while `grant` aliases `workspace` and `work` aliases `pass`.
