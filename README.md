...
- `contracts/stage_run_canary_evidence.json` is the repo-local controlled StageRun canary evidence fixture. It records refs for candidate grant directions and section variants, reflection/review, ranking/selection, revision lineage, meta-review learning, an independent fundability/review gate, owner receipt closeout, a scoped operator summary, and an explicit overclaim boundary; its scope is `controlled_fixture_not_live_domain_progress`.
- The controlled canary operator summary only reports fixture refs and current-owner-delta shape. It does not claim live domain progress, stage completion, grant readiness, fundability readiness, authoring quality readiness, artifact/package freshness, submission readiness, production readiness, or external submission authorization.
...
- `contracts/stage_run_kernel_profile.json` carries a `legacy_runtime_residue_guard` for the old local runtime surfaces. Those residues are history, diagnostic, provenance, refs-only adapter, or tombstone material only; a canary fixture or conformance pass cannot close residue retirement or resurrect a MAG-owned scheduler, attempt ledger, session store, workbench, generic runner, or workflow wrapper.
...
- To inspect the OPL Foundry Agent series command surface, use `<med-autogrant-repo>/scripts/run-python-clean.sh -m med_autogrant.cli foundry status --format json` or a refreshed `medautogrant foundry status --json` wrapper. `mag` remains the package/brand shorthand, but it is not PATH-safe readiness evidence on macOS because other tools may own the bare command. The help first screen identifies `Series: OPL Foundry Agent`, `Agent id: medautogrant`, the ordinary path `workspace/work/stage/run/vault/handoff/connect`, the executable command surface, and the MAG authority boundary. `status`, `inspect`, `interfaces`, `validate`, `doctor`, and `peers` are top-level shortcuts, while `grant` aliases `workspace` and `work` aliases `pass`.
...
- Use `./scripts/verify.sh` for the default local check. It runs the advisory line-budget report, the minimal smoke lane, and the fast non-regression core lane.
...
- Use `./scripts/verify.sh meta`, `./scripts/verify.sh structure`, and `./scripts/verify.sh full` for repo governance, advisory architecture checks, and clean-clone/full-suite baselines. Use `make test-line-budget-strict` or `make test-structure-strict` when daily structure maintenance needs hard enforcement.
