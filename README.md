...
- When an external agent or OPL wants the repo-tracked skill surface directly, use the repo-local clean runner `<med-autogrant-repo>/scripts/run-python-clean.sh -m med_autogrant.cli product skill-catalog --input <input_path> --format json`; it returns one Med Auto Grant app skill plus the underlying command contracts without writing Python artifacts into the checkout.
