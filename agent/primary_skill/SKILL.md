...
OPL canonical agent id 与 OPL Agent Package id 都固定为 `mag`。`med-autogrant` 是仓库、Python distribution、plugin slug、OCI carrier 与安装后 skill locator，不是与 `mag` 并存的 package identity 或兼容 alias。
