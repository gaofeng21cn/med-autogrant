...
The machine-readable implementation boundary is `contracts/pack_compiler_input.json#/implementation_profile`: the declarative pack uses Markdown/JSON, and Python is only a domain-helper language under the declared source root. Minimal authority functions remain declared by the pack authority contracts. This profile does not authorize a generic runtime, CLI, MCP, product-entry, status, or workbench surface.
