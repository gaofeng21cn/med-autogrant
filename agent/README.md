...
`agent/prompts/` holds stage operating prompts: goals, refs, handoffs, and blocker shapes. `agent/professional_skills/` holds repo-local Codex skills for grant-specialist judgment such as call fit, fundability, aims design, section authoring, review, rebuttal planning, and package audit. `agent/tools/` describes bounded affordances only.
