...
- The StageRunController has materialized an identity-bound `opl_stage_review_receipt`, and MAG owner authority has consumed that receipt, current declared scope evidence, and separate release-integrity evidence for the current package refs before recording a local export/readiness verdict; or
...
- Independent gate must confirm that every declared semantic scope is current, package completeness/provenance/portal boundaries are explicit, and release integrity separately matches the exact current four-artifact generation.
- Required gates: `export_and_package`, `quality`, and `authority_boundaries`.
- Package existence, lifecycle completion, provider completion, producer self-check, or helper success cannot declare submission-ready export. Hash drift alone is not content authority: governance metadata, layout, or package-only changes do not invalidate content, methodology, or reference evidence unless their declared dependencies changed. Reviewer/re-reviewer closeout can support scoped Review evidence, but local readiness still requires a separate MAG-owned export/owner verdict and release-integrity evidence.
