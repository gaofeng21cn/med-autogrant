# Med Auto Grant Domain Specialization

Owner: `med-autogrant`
Purpose: grant-authoring specialization of the OPL standard-agent AI-first principle pack.
State: `active_domain_specialization`
Machine boundary: human-readable domain specialization. Machine-readable adoption is in `contracts/standard-agent-principles-adoption.json`; MAG contracts, source, receipts, and verification outputs remain authoritative for MAG behavior.

MAG adopts the OPL principles as a grant-authoring Foundry Agent:

- Intake is the `call_and_candidate_intake` stage and its prompt, not a standalone Skill. It turns a funding opportunity and candidate material into scoped grant material refs, owner questions, typed blockers, route-back refs, or handoff packets.
