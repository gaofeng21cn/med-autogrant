...
selected subset with the current package/grant refs, `source_pack_ref`, and
reference epistemic scope. If the Provider is not usable, stop this MAG Stage
with the contract-defined dependency diagnostic. Provider outputs are refs-only
`candidate_refs`, `owner_gate_handoff_ref`, or `route_back_candidate`; MAG must
consume, reject, or route them back.
