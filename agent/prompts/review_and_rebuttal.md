...
available, compatible, material Skills with the reviewed grant artifact ref,
`source_pack_ref`, and current epistemic review scope. Their outputs are
refs-only `candidate_refs`, `owner_gate_handoff_ref`, or
`route_back_candidate`; the decisive MAG Attempt must consume, reject, or
route them back before issuing its route or quality judgment. Missing or
incompatible Provider state records only a diagnostic or advisory quality hint
and cannot block install, Stage route, operational readiness, independent
review, or create a typed blocker.
