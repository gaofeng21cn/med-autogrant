...
callability before MAG work, then invoke the selected subset with the reviewed
grant artifact ref, `source_pack_ref`, and current epistemic review scope. If
the Provider is not usable, stop this MAG Stage with the contract-defined
dependency diagnostic. Provider outputs are refs-only `candidate_refs`,
`owner_gate_handoff_ref`, or `route_back_candidate`; the decisive MAG Attempt
must consume, reject, or route them back before issuing its route or quality
judgment.
