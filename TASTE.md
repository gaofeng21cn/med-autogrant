...
对 MAG grant、application package、review response 或 funder-facing delivery 任务，真实进度按申请包、叙事/预算/证据 delta、owner decision、human gate、可接力 artifact delta 或用户可审阅结果计算；runtime、queue、stage、read model、测试、docs、provider liveness 和平台修复只是支持证据。MAG/OPL 执行基座是首选路径，不是 grant 推进的前置条件：基座顺畅时走基座，基座卡住时，前端执行者继续推进合法的申请包或资助交付增量，并把暴露出的基座问题作为 side repair lane 记录或修复。只有继续动作会越权写 grant/application authority surface、缺少必要 source/data、或同一写集 ownership 冲突且无法隔离时，才暂停具体目标动作。repair lane 不能吞掉 grant 主线，除非用户目标本身就是平台修复。
