...
| App/operator/release consumption | `manifest_consumer_evidence_landed_sustained_consumption_open` | Product-entry manifest 已默认暴露 `owner_payload_response` 与 `workspace_receipt_scaleout_evidence`，让 OPL/App/operator default manifest path 可发现 MAG owner-payload summary；`manifest_consumer_evidence` 已固定默认 manifest consumer 消费的 refs、counts 和 human-gate blocker path，并保持 `operator_payload_submitted=false`、`claims_sustained_app_consumption_complete=false`。关闭条件仍是 OPL/App/operator closeout、executor-first bundle、release/default caller 持续消费 MAG package refs、quality refs、manual portal boundary、transition oracle refs、safe action refs 和 `authority owner-payload-response` 的 owner receipt success refs path、stage expected receipt / monitor freshness / runtime event refs、typed-blocker payload path。 |
