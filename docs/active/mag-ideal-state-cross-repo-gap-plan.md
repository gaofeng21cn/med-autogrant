...
2026-05-19 的 standard pack 合同校准把根层 `pack_compiler_input` 收为 OPL scaffold canonical 形状：`canonical_semantic_pack_root="agent/"`、`canonical_semantic_pack_role="repo_source_declarative_grant_pack"`，并从 `required_domain_pack_paths` 移除 `agent/README.md`。README 仍可作为人读入口存在，但不能作为机器 required semantic pack path；OPL scaffold validation 只接受真实 prompt / stage / skill / quality gate / knowledge 文件作为 pack compiler 输入。`contracts/runtime-program/current-program.json` 与 `contracts/runtime-program/opl-family-contract-adoption.json` 仍保留 `standard_domain_agent_skeleton.canonical_repo_source_semantic_pack` 的历史字段名，当前只作为 runtime-program snapshot / adoption provenance 读取；后续需要 source-ref refresh 到 canonical 字段，不能反向定义 root pack compiler shape。
