...
2026-07-06 OPL caller hygiene follow-up：OPL Connect module exec 的 MAG caller 已在 `one-person-lab` lane `cc1f46f0b` 从 `uv run --directory <checkout> medautogrant ...` 改为 `<checkout>/scripts/run-python-clean.sh -m med_autogrant.cli ...`，与 MAG repo-local clean runner / grant-native CLI 入口对齐。这只关闭 OPL module exec caller parity 缺口，不授权 MAG 删除仍有 active handler/adapter caller 的 retained surfaces，也不替代 App/workbench sustained consumption、owner receipt / typed blocker roundtrip、continuous no-forbidden-write 或 explicit physical-delete owner receipt。
...
| `generated_default_caller_cutover` | OPL generated/default caller replacement 已有结构证据；retired/tombstone surface 的 no-active-caller evidence 已进入 `functional_privatization_audit.json` 与 `runtime-program/opl-family-contract-adoption.json`。OPL Connect module exec caller 已切到 MAG clean runner，关闭 `uv run medautogrant` 这条不标准 caller hygiene 缺口；`source-purity` strict readback 现在还暴露 body-free `owner_delta_work_order_pack`；当前 `cleanup_candidate_count=0`、`owner_delta_route_count=0`，旧 7 个面已按 strict readback 转为 migrated non-candidate 或 retained current thin surfaces。production default caller、direct/hosted parity 的其他入口、owner receipt roundtrip、continuous no-forbidden-write 和 App/workbench consumption 仍未授权 MAG 物理删除。 | private inventory path-level table、functional privatization audit、runtime-program adoption contract、production acceptance contracts、focused tests、OPL `cc1f46f0b` |
