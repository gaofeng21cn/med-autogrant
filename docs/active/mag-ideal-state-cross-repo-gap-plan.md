...
2026-07-07 OPL payload wrapper thinning：OPL owner payload response 与 manifest sustained-consumption payload response 已从 `ProductEntryEvidenceMixin` pass-through surface 收回 leaf module；CLI 仍保留相同 command output，但不再经 ProductEntry public wrapper 中转。当前剩余 ProductEntry evidence wrappers 限于 domain memory、owner/lifecycle receipt writer 与 production acceptance active authority paths；这些不从 refs-only evidence 推导删除。
