...
Purpose: 对照 MAG 理想目标态和 OPL family 主参考，维护 MAG 当前差距、OPL Framework 应上收的通用能力、production evidence 缺口和后续完善顺序。
State: `active_plan`
...
- OPL closeout 已经能观察到 MAG stage closeout packet、owner receipt refs、controlled no-regression evidence refs、lifecycle receipt refs 与 accepted/rejected memory writeback receipt refs；这证明 receipt evidence path 与 OPL ref-only read model 可用，但还没有把真实 grant-stage long-run soak 写成完成。
...
| Grant transition table | MAG 理想上声明 domain transition spec，OPL 只执行通用 runner | `product-entry-manifest.grant_transition_oracle` 已落地 MAG-owned transition table / oracle fixtures，覆盖 call intake、fundability blocker、specific aims、proposal authoring、review repair、quality closure 和 package human-gate 状态；OPL 已能只读 ingest 该 oracle 并通过 generic matrix runner 做 oracle smoke；OPL family runtime 现在也能把 `family_transition_matrix_result` hydrate 成 provider-hosted `family_transition/domain_tick` task | 真实 grant-stage owner receipt、retry/dead-letter、no-regression evidence 和 live receipt 对账还未闭合；OPL matrix smoke / transition task bridge 不能写成 grant-stage production soak | 用已可 ingest / hydrate 的 oracle 跑真实 OPL provider-hosted transition attempt；MAG 继续只返回 owner receipt、typed blocker 或 no-regression evidence，不让 OPL 判断 fundability / quality / export ready |
...
- owner receipt contract、controlled domain-memory receipt evidence writer、lifecycle guarded apply proof、grant transition oracle table / oracle fixtures；
...
- 真实 OPL-hosted grant-stage attempt 的长时 soak、持续 no-regression evidence 与 receipt 对账；
...
目标：把 OPL 能托管 MAG 从 controlled receipt/no-regression evidence 推进到长时 live soak 和稳定对账。
...
- OPL closeout 中 MAG domain breakdown 持续出现 owner receipt ref，并能观察 no-regression evidence ref 或明确 typed blocker；
...
3. 用已被 OPL descriptor / matrix runner 只读消费并可 hydrate 进 provider-hosted transition task 的 `grant_transition_oracle` 跑真实 transition attempt 与 live receipt 对账；MAG 不新建自己的通用 runner。
