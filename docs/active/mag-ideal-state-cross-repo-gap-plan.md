...
3. 把 MAG 已固化的 `grant_transition_oracle` 接入 OPL generic runner / transition matrix runner；接入前 MAG 不新建自己的通用 runner。
