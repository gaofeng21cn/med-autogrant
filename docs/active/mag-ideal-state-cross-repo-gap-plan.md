...
2026-07-07 OPL/App followthrough refresh：OPL Connect module exec caller 已与 MAG clean runner 对齐；`one-person-lab` default-caller read model 对 MAG 显示 8/8 surface retirement gates closed、deletion evidence worklist `0`、all deletion evidence requirements observed；`runtime app-operator-drilldown --detail full` 显示 MAG manifest sustained-consumption followthrough verified ledger receipt `1`、open workorder `0`。这关闭 OPL 结构/refs-only followthrough 缺口，但不授权 physical delete：OPL read model 仍固定 `physical_delete_authorized=false`、`default_caller_delete_ready=false`，下一步只能由 MAG owner 返回 `physical_delete_authorization_ref`、`keep_as_authority_adapter_ref` 或 `typed_blocker_ref`。

2026-07-07 product-entry manifest cleanup refresh：`opl_substrate_adapter_export` 已从 `locals()` 隐式上下文收薄为显式 manifest 字段传递；这是 product-entry shell 的 source cleanup，不是 product/status/domain-handler 大 shell 物理删除。后续删除仍由 OPL generated/default caller parity、no-active-caller、no-forbidden-write、tombstone/provenance 与 MAG owner physical-delete / keep / typed-blocker 裁决共同 gate。
