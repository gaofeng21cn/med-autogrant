...
| Manifest sustained-consumption payload response | `payload_response_snapshot_landed_cli_followthrough_fail_closed_regression_projected_not_closeout` | `authority manifest-consumption-payload` 校验真实 App/operator 或 release default caller 提交的 sustained-consumption payload，只接受 success refs path 或 typed-blocker path，拒绝未声明 operator payload 字段，并输出 body-free response 给 OPL refs-only record/verify。`contracts/production_acceptance/mag-manifest-sustained-consumption-evidence-20260528.json` 已记录一次 success refs path payload response，但 `long_soak_or_typed_blocker_ref` 仍是 provider long-soak window typed blocker；本轮新增 `provider_long_soak_followthrough`，把该 ref 分类为 typed blocker、暴露空 long-soak evidence refs，并固定 `claims_provider_long_soak_complete=false` / `closes_provider_long_soak=false`。非 mocked grouped CLI 回归已覆盖 parser/handler/product-entry 串联，证明 direct CLI/default-caller payload path 会返回同一 provider follow-through shape，并在 CLI 层 fail-closed 拒绝未声明 operator 字段以及 success refs path / typed blocker path 混用；同一事实已折回 production acceptance snapshot 的 `grouped_cli_regression_evidence` 字段。该 snapshot 与 CLI regression 不生成 operator payload、不创建 MAG owner receipt、不关闭 App sustained-consumption、human gate、submission-ready 或 Temporal long-soak。 |
...
| Foundry Agent series pin contract | `done_for_structural_conformance` | `contracts/foundry_agent_series.json`、`src/med_autogrant/opl_standard_pack.py`、`tests/test_opl_standard_pack.py`、OPL family conformance | `contract_version_policy` 与 `shared_release_pin_strategy` 已对齐 OPL standard scaffold；这只关闭 structural blocker，不授权 domain ready、quality/export verdict、owner receipt 或 default promotion。 |
