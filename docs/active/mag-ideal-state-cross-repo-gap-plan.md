...
- `grant_autonomy_controller.py` 只能作为 grant route / budget / blocker policy 的 thin direct handler；`grant_autonomy_loop_shell.py` 是 OPL attempt lifecycle / generated operator loop 的迁移输入，不得变成 MAG-owned long-running runtime loop。质量/fundability/export 判断必须来自 AI-first grant stage artifact。
