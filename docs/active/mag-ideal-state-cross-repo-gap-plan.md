...
本文不把这些基础面解释成“MAG 现有结构已经足够，只需补证据”。目标态是标准 OPL Agent。MAG 仍有手写 shell、local ledger、projection/helper 和 product wrapper 时，应优先问它是否能由 OPL pack compiler / generic primitive / App shell 承接；能承接就列为功能/结构差距，不能承接才写清 grant authority 边界。真实 workspace receipt、long soak 和 App drilldown 只用于验证目标结构，不用于为历史私有平台续命。
