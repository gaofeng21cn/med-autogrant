...
- `当前实态`：MAG 没有发现类似 MAS local LaunchAgent scheduler 的默认运行 owner 残留；当前差距集中在 OPL generic primitive handoff、adapter thinning、真实 workspace memory/package/lifecycle receipts、scaffold 抽取和 live receipt 对账。
- `最短路径`：先做 OPL generic primitive absorption design and MAG adapter thinning，再做 package/export lifecycle、grant strategy memory、skeleton/template extraction 和 legacy direct retirement。
- `验收顺序`：focused OPL-hosted receipt verification 在功能边界收薄之后执行；long-run live soak 是最后的 production closure，不是 P1-P4 的前置条件。
...
2026-05-16 consumer/thinning lane 进展：MAG 侧已把上述边界落成 `product-entry-manifest.mag_consumer_thinning_contract`，并由正确入口 `uv run python -m med_autogrant.cli product sidecar-export --input examples/nsfc_workspace_p2c_critique.json --format json` 投影。该合同固定 MAG adapter 只输出 grant-owned refs、owner receipt、typed blocker、verdict refs 和 domain action metadata，并声明消费 OPL `family_scheduler_replacement`；workspace/source intake shell、memory locator/writeback transport、package/export lifecycle shell、generic transition runner、operator workbench/observability/SLO 和 agent scaffold checklist 都是 OPL replacement expectation。MAG 不持 generic scheduler、daemon、lifecycle owner、queue、attempt ledger、generic state-machine runner 或 generic workbench owner 角色，且 sidecar fresh output 明确 `claims_opl_replacement_exists=false`、`claims_production_long_run_soak_complete=false`。同一合同现在还暴露 `thin_surface_output_guard` 与 `standard_agent_scaffold_alignment`，分别证明 sidecar 只能回传 grant refs / receipts / blocker / verdict refs / action metadata，并证明 MAG 保留必要 domain entry、schema、sidecar 和 focused tests，不退化为纯知识文件仓。该进展只关闭 MAG 侧 P1 adapter-thinning machine surface，不关闭 OPL replacement consumption、真实 workspace memory/package/lifecycle evidence、focused hosted receipt verification 或 live soak。
...
这份计划的核心判断是：MAG 后续不应继续在仓内重复建设通用运行外围；应把可复用的 transport、lifecycle、projection、operator workbench 能力上收到 OPL Framework，MAG 仓保持 grant-domain authority pack 和 domain package 薄程序面。
...
| OPL-hosted MAG path | OPL 托管 stage attempt，MAG 返回 domain receipt、typed blocker 或 no-regression evidence | OPL 可解析 MAG descriptor/stages/memory，已能从 MAG sidecar closeout 观察 owner receipt refs 与 controlled no-regression evidence refs | generic primitive handoff、adapter thinning、receipt 对账和真实 grant-stage long-run soak 仍未闭合 | 先完成 OPL primitive absorption 与 MAG thin adapter 收敛，再用 focused hosted attempt 验证 owner receipt / typed blocker / no-regression evidence，最后扩展到 live soak |
...
| Physical skeleton | repo-source 按 `agent/ contracts/ runtime/ docs/` 清晰分层，workspace artifacts 不进 source | MAG 已有 minimum repo-source anchors，OPL 读模型识别为 evidence observed；`agent/`、`runtime/` 当前是 descriptor/index anchor，不是完整物理迁移 | 更大范围物理迁移仍需 direct/hosted parity、restore/provenance proof 和 no-forbidden-artifact-blob proof | 按最新 owner surface 迁移；发现旧 import/caller/test 时直接改到新 surface，不保留 path compatibility shell |
| New-agent template readiness | 新 OPL Agent 应能按统一 docs taxonomy、descriptor/stage/action/memory/artifact locator、sidecar/receipt schema 和 no-forbidden-write gate 建仓 | MAG 已具备 `agent/`、`contracts/runtime-program/`、`plugins/mag`、`runtime/`、`schemas/`、canonical docs taxonomy 和 domain package 薄程序面 | 目录仍混有 MAG-specific Python package、grant schemas、historical proof surface 和 artifacts/proof 输出；不能直接复制为通用 scaffold | 抽取 template 文档/contract checklist 到 OPL；MAG 保持 grant-specific 实现，后续只把可复用 scaffold 规则上收到 OPL。 |
...
这组上收不改变 authority：OPL 提供壳、transport、ledger、projection 和 UI；MAG 继续持有 domain truth、quality/export verdict、artifact/memory authority 和 domain package 薄程序面。
...
目标：先把 MAG 已明确不应长期持有的通用外围能力设计成 OPL owner primitive，并把 MAG 侧 adapter 收薄；不等待真实 grant-stage live soak 才开始。

动作：

1. OPL 定义 workspace/source intake shell，MAG 只提供 funding call/profile/task-lock adapter、domain blocker 和 owner receipt。
...
3. OPL 定义 package/export lifecycle shell，MAG 只提供 package refs、gap report、submission-ready verdict、manual portal boundary 和 owner receipt。
4. OPL/App 定义 route/decision graph、quality/readiness panel、attention queue、repair commands 和 observability/SLO；MAG 只输出 route nodes/edges/rationale refs、AI critique-backed verdict、hard blockers 和 safe action refs。
5. MAG manifest/sidecar 保持 thin adapter 角色：只输出 refs、receipt、typed blocker、verdict refs 和 domain action metadata，不复制 OPL runtime、queue、attempt ledger、generic runner 或 App workbench logic。
6. 所有 generic primitive 都要求 authority boundary 字段：`opl_can_write_domain_truth=false`、`opl_can_write_memory_body=false`、`opl_can_declare_export_ready=false`。
...
- App/workbench 可以只读看到 package state、gap report、owner、freshness、manual portal boundary；
...
目标：在功能上收、adapter thinning、memory/package/lifecycle handoff 和 skeleton cleanup 完成后，用 focused hosted attempt 验证 MAG owner receipt / typed blocker / no-regression path。
...
1. 先与 OPL 对齐 generic primitive absorption 的最小切片：memory locator/writeback transport、package/export lifecycle shell、quality/readiness projection shell 和 operator observability/SLO。
2. 把 MAG sidecar/direct adapter 收薄，只输出 grant-owned refs、receipt、typed blocker、verdict refs 和 domain action metadata，不复制 OPL runtime/workbench。
...
5. 抽取可复用 scaffold/template checklist 到 OPL，并按 latest owner surface 推进 MAG legacy cleanup；旧模块、旧接口、旧测试已被替代就直接清理或归档。
6. 再选定一个低风险真实 grant-stage line，用已可 ingest / hydrate 的 `grant_transition_oracle` 跑 focused hosted receipt verification 与 live receipt 对账。
7. 最后扩展到 long-run live soak；live soak 是生产验收，不再作为上面功能迁移、上收、清理的前置条件。

完成上述步骤后，MAG 才能从 “OPL-compatible descriptor / controlled evidence available” 进入更接近 “production-hosted grant agent” 的状态。
