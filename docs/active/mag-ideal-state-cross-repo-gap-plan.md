...
| OPL owner-payload response | `return_shape_landed_scaleout_pending` | `authority owner-payload-response` 现在输出 OPL owner-payload workorder 可消费的 `domain_owner_receipt_refs`、`owner_chain_refs`、`no_regression_evidence_refs`、`typed_blocker_refs`、success / typed-blocker payload path、required return shapes 和 legacy alias；`authority receipt-readiness` 是它的当前 MAG-owned receipt refs 前置入口。2026-05-26 fresh CLI proof 重新生成了四类 receipt refs readiness，并得到 `blocked_by_submission_ready_human_gate` owner-payload response；`submission_ready_export_gate` typed blocker 仍是 blocker，不是 approval 或 ready verdict。 |
