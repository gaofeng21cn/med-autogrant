...
2026-07-07 OPL/App followthrough refresh 与 MAG owner 裁决：OPL Connect module exec caller 已与 MAG clean runner 对齐；`one-person-lab` default-caller read model 对 MAG 显示 8/8 surface retirement gates closed、deletion evidence worklist `0`、all deletion evidence requirements observed；`runtime app-operator-drilldown --detail full` 显示 MAG manifest sustained-consumption followthrough verified ledger receipt `1`、open workorder `0`。这关闭 OPL 结构/refs-only followthrough 缺口，但不授权 physical delete：OPL read model 仍固定 `physical_delete_authorized=false`、`default_caller_delete_ready=false`。MAG owner 已在 `contracts/private_functional_surface_policy.json#/physical_source_morphology_policy/retirement_readback_cleanup_guard/compact_cleanup_readiness_summary/retained_surface_owner_decision` 返回 `keep_as_authority_adapter_ref` 裁决；当前 retained surfaces 读作 `retain_as_current_thin_domain_target`，不再等待 owner physical-delete / keep / typed-blocker 裁决，也不声明 deletion-ready。

2026-07-07 product-entry manifest cleanup refresh：`opl_substrate_adapter_export` 已从 `locals()` 隐式上下文收薄为显式 manifest 字段传递；这是 product-entry shell 的 source cleanup，不是 product/status/domain-handler 大 shell 物理删除。当前 MAG owner 裁决是 keep current thin authority adapters；后续若要物理删除，仍必须有新的 explicit physical-delete authorization 或 typed blocker/tombstone 裁决。

2026-07-07 wrapper/test portfolio thinning：本轮已吸收 grouped CLI 未注册 handler 删除、`receipt-readiness` leaf builder 直连、receipt/readiness evidence wrapper 退役，以及 active-path / functional-closure 结构测试从完整历史清单锁定改为 contract sentinel。已关闭的是 repo-local wrapper 与测试镜像锁定债务；未关闭、也不能从该轮推导关闭的是 retained `product_entry` / `status` / `user_loop` / `domain_handler` / `control_plane` / `lifecycle` 物理删除。OPL 侧 default-caller keep authority closeout 已明确 `keep_as_authority_adapter_ref` 不再需要 OPL default-caller deletion worklist，但仍固定 `physical_delete_authorized=false` 与 `default_caller_delete_ready=false`。

2026-07-07 evidence read-model wrapper thinning：controlled-soak observability、stage-attempt observability、hosted receipt verification、external evidence ledger、controlled-soak reconciliation proof/inventory 与 continuous reconciliation snapshot 已从 `ProductEntryEvidenceMixin` pass-through surface 收回 leaf module；独立 OPL conflict envelope leaf builder 已退役，当前 envelope kind/schema 只由 consumer-thinning manifest projection 承载。这减少测试保活的第二入口，不改变 owner receipt signer、CLI authority command、domain handler closeout 或 retained big-shell physical-delete gate。

2026-07-07 retained shell cleanup：domain-handler 的 shell/projection helper namespace 已收回单一 active caller，`grant_direct_entry` 纯转调 runtime surface wrapper 已删除，测试组合不再固定 helper import/re-export 位置；后续若要删除 manifest_shell、lifecycle/readback projection 或 consumer_thinning audit，必须先迁移对应 source-ref/current-program contract。

2026-07-07 closeout projection wrapper thinning：codex stage execution receipt bundle、operator closeout readiness、physical morphology guard 与 executor-first closeout bundle 已从 `ProductEntryEvidenceMixin` pass-through surface 收回 leaf module；CLI/domain_handler 仍保留相同 command/result shape，但不再经 ProductEntry public wrapper 中转。这减少 ProductEntry generic shell 面积，不改变 production acceptance、owner receipt signer、grant truth 或 retained big-shell physical-delete gate。

2026-07-07 evidence wrapper closeout：OPL owner payload response、manifest sustained-consumption payload response、production-live-acceptance receipt projection、domain memory writeback 与 owner/lifecycle receipt writer 已从 `ProductEntryEvidenceMixin` pass-through surface 收回 leaf module；CLI/domain_handler 仍保留相同 command/result shape，但不再经 ProductEntry public wrapper 中转。`ProductEntryEvidenceMixin` 已退役；domain memory、owner/lifecycle receipt writer 与 production acceptance leaf modules 仍是 retained authority/write/projection surfaces，不从 refs-only evidence 推导物理删除。

2026-07-10 shared primitive owner migration：MAG 已删除私有 `workspace_scaffold.py`、`schema_subset_validator.py` 与 `product_entry_parts/runtime_registration.py` 通用实现，caller 直接消费 `opl_harness_shared` 的 workspace boundary、schema traversal、product preflight、runtime registration、owner-evidence reconciliation/observability 与 artifact lifecycle primitive。MAG 只保留 declarative workspace/runtime/lifecycle profile、grant runtime constraints、receipt/typed-blocker/human-gate authority、submission/export verdict normalization 与 package authority；`family_shared_release.py` 不再用 MAG 私有 commit 特判覆盖 OPL owner release contract。该迁移关闭对应 source-level generic implementation residue，不关闭其他 retained shell 的删除门，也不构成 grant-ready、submission-ready、production-ready 或 physical-delete-ready 证据。
...
当前 `product_entry*`、`product_entry_parts/*`、`domain_runtime*`、runtime registration、domain-handler、lifecycle、memory/package projection、autonomy loop、status/user-loop shell 与部分 CLI/rendering shell 仍可见于 active source。它们只能按 direct handler、refs-only adapter、minimal grant authority function、diagnostic 或 migration input 读取，不能重新写成 MAG 私有 runtime platform；按 strict purity，它们已进入 OPL replacement-ready / physical-delete-not-authorized 状态。当前 `product_entry_parts/manifest_builder.py`、`entry.py` 与 `grant_direct_entry.py` 已统一收口到 `runtime_surfaces._build_default_runtime_continuity_surfaces()`；其中 manifest builder 只通过 override command 注入 current shell command，不再单独手写 runtime summary / managed runtime / continuity quartet 组装。`product_entry_parts/loop_route_shell.py` 则复用 `domain_runtime_parts.contracts.build_author_side_route_command()` 的 canonical route-command map，避免在 product/user-loop shell 再维护第二份 landed route command truth。旧 product-sidecar / sidecar CLI wrapper 已从 active caller 口径移除，只能作为 no-resurrection guard 或 history/provenance 词汇出现。
...
| External evidence request accounting | `refs_only_closed` | `mag-evidence-receipt-ledger.json` 与 OPL MAG sustained-consumption ledger 持有 request/followthrough refs-only closure 摘要；active plan 不冻结 request/receipt 计数。剩余真实证据门是 `temporal_provider_long_soak_window_evidence`、human gate / W7 closing ref 和 MAG physical-delete owner 裁决。 |
| Standard OPL Agent source shape | `shared_primitives_consumed_physical_delete_tail_remaining` | `agent/`、stage control plane、quality gates、pack compiler input 和 MAG owner boundary 已是当前默认语义结构；workspace/schema、runtime registration、owner-evidence reconciliation/observability 与 package lifecycle 通用实现已迁至 OPL shared，MAG 只保留声明 profile 与 domain authority adapter。OPL default-caller replacement/cutover readiness 已观测，但 product/status/user-loop/domain-handler/domain_runtime/autonomy/CLI target 仍作为 handler/adapter target 暂留，不声明 grant-ready、submission-ready、production-ready 或 physical-delete-ready。 |
...
| Evidence accounting surfaces | `refs_only_followthrough_observed_not_authority` | external ledger、owner-payload response、workspace receipt scaleout、manifest sustained-consumption、stage replay/monitor projection 均按 body-free refs-only / count-only provenance 读取；OPL/App/default-caller followthrough 已有 verified refs-only 读回，但 human gate、stage success-rate、Temporal long soak 和 MAG physical-delete owner 裁决仍开放。 |
...
按 strict standard-agent purity，workspace boundary、schema traversal、runtime registration、owner-evidence reconciliation/observability 与 package lifecycle envelope 的 source-level generic implementation residue 已经关闭；其余 MAG active source 已完成 replacement-ready 分类，但未获得 physical delete authority。`mag_functional_structure_gap_count=0` 只表示机器分类与本批 owner migration 闭合，不代表 active shell 已清零。product/status/user-loop/domain-handler/domain_runtime/autonomy/CLI handler/adapter shell 的删除门不变；`generated_surface_bridge_exit`、`legacy_runtime_session_lifecycle_exit`、`package_memory_lifecycle_refs_only_boundary`、`private_authority_ai_first_guard` 和 `physical_morphology_tail` 必须继续守住 OPL generated/default caller 接管证据与 repo-local wrapper 删除门。
...
| `generated_default_caller_cutover` | OPL generated/default caller replacement、no-active-caller、tombstone/provenance、owner receipt / typed blocker roundtrip、continuous no-forbidden-write 与 App/workbench sustained-consumption followthrough 已按 OPL refs-only 读面 observed。OPL Connect module exec caller 已切到 MAG clean runner，关闭 `uv run medautogrant` 这条不标准 caller hygiene 缺口；`source-purity` strict readback 现在暴露 `observed_non_authorizing_evidence` 与 `remaining_authority_gap`。当前 `cleanup_candidate_count=0`、`owner_delta_route_count=0`，旧 7 个面已按 strict readback 转为 migrated non-candidate 或 retained current thin surfaces；仍未授权 MAG 物理删除，下一步只剩 MAG owner 三选一裁决。 | private inventory path-level table、functional privatization audit、runtime-program adoption contract、production acceptance contracts、focused tests、OPL `agents default-callers`、OPL `runtime app-operator-drilldown` |
| `opl_non_default_executor_receipt_tail` | `hermes_agent` critique 路径只作为 OPL-owned non-default executor receipt lane 读取；MAG 只声明/消费 request、receipt、closeout 与 critique payload vocabulary，不管理 Hermes executor substrate，也不保留 MAG-owned helper/runtime implementation。repo-local helper/runtime 若仍有 caller，只是迁出/删除候选；关闭门是 OPL adapter 提供等价 request/receipt entry、MAG caller 切走、focused receipt tests 改到 OPL-owned surface、no-active-caller scan 与 MAG owner receipt / typed blocker。 | executor receipt source/tests、OPL adapter owner evidence、history/provenance |
...
本文不维护 ready / production evidence worklist。grant-ready、fundability-ready、quality-ready、export-ready、submission-ready、Temporal long-soak、W7 owner evidence 和 human-gate receipt 只在单独 evidence lane 或 owner contract 中读取；App/operator/default-caller sustained-consumption followthrough 已作为 refs-only evidence observed，但不得反向写成 MAG ready、production ready 或 physical-delete authorization。
...
- Required actions:
  ...
  2. 保持 product/status/user-loop/domain-handler/CLI shell 的 refs-only adapter 或 direct
     handler target 边界；workspace/schema/runtime-registration/owner-evidence/artifact-lifecycle
     通用逻辑直接消费 OPL shared primitive，只在 MAG 保留声明 profile 与 domain authority
     adapter；新增能力优先服务 OPL generated/default caller parity，不扩写 MAG-owned shell。
