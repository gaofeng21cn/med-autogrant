...
3. 用已被 OPL descriptor / matrix runner 只读消费并可 hydrate 进 provider-hosted transition task 的 `grant_transition_oracle` 跑真实 transition attempt 与 live receipt 对账；OPL workbench 已能展示 transition bridge evidence refs，MAG 不新建自己的通用 runner。
