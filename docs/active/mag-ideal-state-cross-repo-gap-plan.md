...
3. 把已被 OPL descriptor / matrix runner 只读消费的 `grant_transition_oracle` 接到真实 provider-hosted transition attempt 与 live receipt 对账；MAG 不新建自己的通用 runner。
