...
- `当前实态`：MAG 没有发现类似 MAS local LaunchAgent scheduler 的默认运行 owner 残留；MAG 作为 OPL standard scaffold / generic primitives consumer 的基础面已经落到 manifest、sidecar projection、schema 和 focused guard。2026-05-17 后目标收紧为 OPL pack compiler / generated surface：MAG repo-local refs-only surface 已覆盖 focused hosted receipt verification、memory receipt read projection、package lifecycle handoff projection、cleanup/restore/retention lifecycle receipt bundle 和 continuous receipt reconciliation snapshot。上述能力只能证明迁移输入和 refs-only projection 已存在，不能把 MAG 功能/结构差距写成完成。当前功能/结构差距仍包括 OPL generated surface 尚未生产消费、手写 product/status/user-loop/sidecar/grouped CLI/projection/lifecycle adapter 尚未迁到 generated surface、workspace/source intake shell / memory locator / package export lifecycle / workbench / observability handoff 尚未由 OPL/App 承接，以及 legacy runtime/journal/probe/compat residue 的 no-active-caller physical cleanup 尚未执行完。
- `最短路径`：先把上述 still-active 功能面迁到 OPL generated/hosted surface 或 direct retirement，再用真实 workspace/runtime evidence 验证 memory/package/lifecycle refs 被 OPL/App shell 消费，随后做持续 live receipt reconciliation，最后做 long-run live soak。
- `验收顺序`：generated surface production consumption、active caller cutover、App/workbench drilldown、package/export lifecycle shell、memory/package/lifecycle handoff 和 legacy physical cleanup 属于功能/结构差距；真实 grant workspace receipt、long-run soak、真实 memory/package/lifecycle receipt、continuous receipt reconciliation 和 no-forbidden-write proof 属于测试/证据差距。
...
本文不把这些基础面解释成“MAG 现有结构已经足够，只需补证据”。目标态是标准 OPL Agent。MAG 仍有手写 shell、local ledger、projection/helper 和 product wrapper 时，应优先问它是否能由 OPL pack compiler / generic primitive / App shell 承接；能承接的通用部分必须通过 declarative grant pack、generated surface handoff、refs-only adapter 或 direct retirement 归位，不能写成 MAG 长期私有 owner。只要 OPL generated surface 尚未被生产消费、active caller 尚未完成 cutover、App/workbench shell 尚未承接 handoff、或 legacy residue 尚未 no-active-caller 删除，就仍是功能/结构差距；真实 workspace receipt、long soak、memory/package/lifecycle receipt 和 no-forbidden-write proof 用于证明这些结构在真实运行中成立。
...
主要差距已经不在概念、命名、descriptor、MAG repo 内 receipt writer、consumer/thin adapter 或声明式 pack 输入。后续计划也不应把 production closure / long soak 放成所有已知工作的前置条件。MAG 当前已经把可复用 workspace/source intake、memory locator/writeback transport、package/export lifecycle shell、generic transition runner、operator workbench、observability/SLO、scaffold/template 规则和 hand-written thin shell 输入声明为 OPL-owned replacement / generated-surface handoff expectation，并把本仓收窄为 grant authority pack、declarative grant pack、minimal authority functions 和 refs-only adapter。之后要完成的功能/结构工作是把这些 handoff expectation 变成实际 OPL/App generated/hosted caller、把手写 shell 迁走、让 App/workbench 承接 drilldown 与 lifecycle shell、并删除 no-active-caller legacy residue；测试/证据工作是真实 grant-stage receipt、真实 memory/package/lifecycle receipt、focused hosted verification、continuous reconciliation、no-forbidden-write proof 和 live soak 验收。
...
| --- | --- | --- | --- | --- | --- |
| MAG direct path | 单一 app skill / CLI / domain entry 能独立完成 grant authoring、review、package 和 owner receipt | direct capability surface 已落地，product status / user-loop / direct-entry / package submission-ready / quality surfaces 可调用；其中 product/status/user-loop/grouped CLI 仍属于 hand-written surface | hand-written direct wrappers 仍需迁到 OPL generated surface 或收薄为 explicit grant authority function；direct path 与 generated path 的 active caller cutover 尚未完成 | 真实 end-to-end grant workspace 的 owner receipt、memory writeback、package lifecycle receipt 和 no-forbidden-write proof 不足 | 先完成 generated caller cutover，再用真实 grant workspace 跑 stage closeout，留下 owner receipt、quality closure、package refs 和 no-forbidden-write proof |
| OPL-hosted MAG path | OPL 托管 stage attempt，MAG 返回 domain receipt、typed blocker 或 no-regression evidence | OPL 可解析 MAG descriptor/stages/memory，已能从 MAG sidecar closeout 观察 owner receipt refs 与 controlled no-regression evidence refs；MAG repo-local focused hosted receipt verification 与 continuous receipt reconciliation snapshot 已落地 | OPL generated surface production consumption、replacement caller migration、App/workbench drilldown 和 hosted lifecycle shell 尚未闭合 | 持续 live receipt reconciliation、真实 grant-stage long-run soak、真实 hosted attempt receipt 和 no-regression proof 仍未闭合 | 用真实 OPL-hosted attempt 生成 owner receipt / typed blocker / no-regression evidence，再扩展到 generated surface consumption、live reconciliation 和 live soak |
| Stage-led grant control plane | 6 个 grant stage 具备输入、prompt/skill、knowledge、quality gate、handoff、closeout | OPL `stages list` 可解析 MAG 6-stage plane，stage descriptor / transition / package / memory / lifecycle handoff refs 已纳入 MAG pack 输入 | stage descriptor 已成立，但 stage attempt ledger、handoff shell、queue/human gate、workbench drilldown 仍需由 OPL generated/hosted surface 生产消费 | 真实 owner receipt、memory/package/lifecycle handoff 与 provider-hosted activity 证据不足 | 对 `fundability_strategy`、`proposal_authoring`、`review_and_rebuttal`、`package_and_submit_ready` 跑 controlled attempts 并对账 receipt |
| Grant transition table | MAG 理想上声明 domain transition spec，OPL 只执行通用 runner | `product-entry-manifest.grant_transition_oracle` 已落地 MAG-owned transition table / oracle fixtures，覆盖 call intake、fundability blocker、specific aims、proposal authoring、review repair、quality closure 和 package human-gate 状态；MAG focused proof 已把 `quality_closed_to_package` oracle fixture 通过 sidecar `stage-attempt/closeout` 写成 owner/no-regression receipt refs；OPL 已能只读 ingest 该 oracle 并通过 generic matrix runner 做 oracle smoke；`controlled-soak-receipt-reconciliation-inventory` 已可把多条 MAG owner receipt evidence 聚合成 read-only reconciliation inventory | OPL generic runner 的 production consumption、transition bridge workbench drilldown 和 active caller cutover 仍未完成；MAG 侧只保留 transition truth 和 owner action | 真实 grant-stage long-run soak 仍未闭合；repo-local no-regression closeout case、OPL matrix smoke、transition task bridge、receipt inventory 或 workbench drilldown 不能写成 grant-stage production soak | 用真实 OPL provider-hosted transition attempt 和 live receipt 对账验证 MAG 只返回 owner receipt、typed blocker 或 no-regression evidence |
| Grant strategy memory | stage 检索少量 memory refs，closeout 生成 proposal，MAG owner accept/reject 并写 receipt | OPL memory descriptor resolved；MAG 已有 descriptor、proposal、accept/reject、runtime receipt evidence writer 和 body-free memory receipt read projection | OPL memory locator/index/writeback transport 尚未生产承接；MAG memory receipt projection 仍只是 handoff surface，active caller 需迁到 OPL/App shell | retrieval/writeback apply、memory body migration、真实 memory receipt 和跨 workspace policy proof 未形成长线闭环 | 把真实 memory body 保持在 workspace/runtime root；OPL 只消费 locator/index/receipt projection 和 freshness |
| Package/export lifecycle | MAG 持 export gate，OPL/App 展示 package refs、gap report、restore/provenance 和 manual portal boundary | `package submission-ready` 是 MAG-owned export gate；MAG manifest 已把 package/export lifecycle shell 声明为 OPL replacement expectation，MAG 只保留 verdict、gap report、package refs、owner receipt，并已提供 package lifecycle handoff projection | OPL/App package/export lifecycle shell、manual portal boundary drilldown、restore/provenance/retention handoff 和 active caller cutover 尚未闭合 | 真实 workspace package/lifecycle receipt 泛化和 OPL/App 外部 shell 生产消费证据仍未闭合 | OPL 提供 package/export shell；MAG 返回 submission-ready verdict、gap report、package refs、manual portal boundary 和 owner receipt |
| Artifact lifecycle | OPL 管 locator/retention/restore ledger，domain artifact mutation 需 MAG receipt | MAG 已有 lifecycle guarded apply proof；consumer contract 明确 MAG 不持 generic artifact lifecycle owner；MAG 已能聚合 cleanup/restore/retention lifecycle receipt bundle | OPL locator/retention/restore ledger 与 App/workbench drilldown 尚未生产承接；MAG lifecycle bundle 仍是 handoff projection | cleanup/restore/retention 的真实 workspace coverage、真实 lifecycle receipt 和 App/workbench drilldown proof 仍不足 | 扩展到真实 grant workspace 的 cleanup/restore/retention guarded receipt，并保持 OPL 只写自有 ledger/locator |
| Quality/readiness projection | App 展示 quality state、issue lineage、closure dossier、next action，但不替 MAG 下 verdict | MAG quality scorecard / closure dossier / diff 已落地 | OPL/App quality/readiness projection shell、issue lineage drilldown 和 action routing 尚未生产承接；MAG 保留 AI critique-backed verdict、issue refs 和 hard blockers | App/workbench 对 MAG issue refs、verdict refs 和 hard blockers 的真实 drilldown 证据不足 | OPL 提供 projection shell；MAG 返回 AI critique-backed verdict、issue refs 和 hard blockers |
| Physical skeleton | repo-source 按 `agent/ contracts/ runtime/ docs/` 清晰分层，workspace artifacts 不进 source | MAG 已有 minimum repo-source anchors，OPL 读模型识别为 evidence observed；consumer contract 通过 `consumed_opl_standard_surfaces` 明确 MAG 消费 OPL standard scaffold | 物理 skeleton 仍只是 minimum anchors；generated surface owner layout、active caller cutover、legacy runtime/journal/probe/compat residue 的 no-active-caller 删除尚未完成 | direct/hosted parity、restore/provenance proof 和 no-forbidden-artifact-blob proof 仍需闭合 | 发现旧 import/caller/test 时直接改到最新 owner surface，不保留 path compatibility shell |
| New-agent template readiness | 新 OPL Agent 应能按统一 docs taxonomy、descriptor/stage/action/memory/artifact locator、sidecar/receipt schema 和 no-forbidden-write gate 建仓 | MAG 已具备 `agent/`、`contracts/runtime-program/`、`plugins/mag`、`runtime/`、`schemas/`、canonical docs taxonomy、grant authority pack 和 declarative pack anchors；MAG 当前声明消费 OPL scaffold/checklist，不持有通用模板 owner | OPL template/checklist 抽取、generated scaffold consumption 和 MAG hand-written scaffold residue cleanup 尚未完成 | OPL 侧 scaffold consumer proof 仍需补齐 | 抽取 template 文档/contract checklist 到 OPL；MAG 保持 grant-specific 实现 |
| OPL pack compiler / generated surface | OPL 应从 MAG descriptor、stage graph、action metadata、receipt schema、policy table 和 authority manifest 生成 CLI/product-entry/sidecar/status/workbench/harness | MAG 已提供 declarative grant pack compiler input、generated surface handoff 与 minimal authority function manifest；当前手写 product status/user-loop/sidecar adapter、grouped CLI/API、projection builder、receipt/lifecycle helpers 只作为迁移桥 | OPL generated surface 尚未 production consumption；hand-written product/status/user-loop/sidecar/grouped CLI/projection/lifecycle adapter 仍需迁到 generated surface 并切 active caller | generated surface no-regression proof、direct/hosted parity proof 和真实 hosted receipt 仍缺 | OPL generated surface 消费 MAG pack input 后，迁移 active caller 并验证 direct MAG surface 不回归 |
| Legacy retirement | 旧 Hermes/Gateway/local-manager active path 只保留 history/proof/provenance | MAG status 已记录 tombstone / physically removed evidence；runtime patch bridge 与旧 product-entry 聚合测试已删除；仍需继续扫描 legacy runtime/journal/probe/compat residue | legacy runtime/journal/probe/compat residue 的 no-active-caller physical cleanup 尚未完成；旧模块/接口/测试一旦无 active owner，必须直接删除或归档 | no-active-caller scan、replacement parity、provenance proof 和 no-forbidden-write proof 仍需补齐 | 旧模块/接口/测试一旦无 active owner，直接删除或归档；只保留必要 provenance，不新增 alias/facade/compat test |
...
当前状态：`mag_consumer_thinning_contract`、`consumed_opl_standard_surfaces`、`functional_harness_consumer_coverage`、`thin_surface_output_guard`、sidecar export、schema 和 focused tests 已经固定 MAG 只消费 OPL standard scaffold / generic primitives / functional harness，不持有 generic scheduler/daemon/queue/attempt ledger/runner/workbench/memory transport/artifact lifecycle，也不把 OPL harness pass 提升成 grant-ready 或 export-ready。这是 handoff 与 guard 基础，不是功能/结构差距完成证明。

当前功能/结构差距：OPL generated surface 仍未生产消费；hand-written product status、user-loop、sidecar adapter、grouped CLI/API、projection builder 和 lifecycle wrapper 仍需迁到 generated surface；active caller cutover 仍需执行。回归验证和真实 hosted receipt 另归测试/证据差距。
...
5. 把 MAG hand-written thin shell 的输入归入 declarative grant pack / generated surface handoff；OPL generated surface 可用后执行 active caller migration，并删除已无 caller 的手写 wrapper。
...
当前状态：MAG 侧 consumer contract 已把 package/export lifecycle shell 和 generic artifact lifecycle 声明为 OPL-owned replacement expectation，并把 MAG 保留内容限制为 package refs、gap report、submission-ready verdict、manual portal boundary、lifecycle receipt bundle 和 owner receipt。`product package-lifecycle-handoff` 与 `product lifecycle-receipt-bundle` 已提供 refs-only grouped CLI/API。剩余功能/结构缺口是 OPL/App 外部 shell 的生产消费、App/workbench drilldown、restore/provenance/retention handoff 和 active caller cutover；真实 workspace lifecycle receipt 泛化归测试/证据差距。

当前功能/结构差距：package/export lifecycle shell 尚未由 OPL/App 生产承接；MAG handoff projection 仍只是 refs-only 桥；cleanup/restore/retention 的 generic lifecycle owner 和 workbench drilldown 仍需切到 OPL/App。
...
当前状态：MAG 侧 consumer contract 已把 memory locator/writeback transport 声明为 OPL-owned replacement expectation，并通过 `consumed_opl_standard_surfaces` 保留 MAG 对 memory body、writeback proposal、accept/reject 和 receipt writer 的 authority。`product memory-receipt-projection` 已把 accepted/rejected receipts 收成 body-free refs projection。剩余功能/结构缺口是 OPL/App memory locator/index/writeback transport 的生产承接和 active caller cutover；真实 workspace/runtime memory body migration、retrieval/writeback apply 和跨 workspace policy 泛化归测试/证据差距。

当前功能/结构差距：memory locator/index/writeback transport 尚未由 OPL/App shell 生产承接；MAG memory projection 仍是 handoff surface；旧 memory descriptor shape、旧 product-status trace 或旧测试 monkeypatch caller 仍需迁移或删除。
...
当前状态：MAG 侧 consumer contract 已通过 `standard_agent_scaffold_alignment` 与 `consumed_opl_standard_surfaces` 固定为 OPL standard scaffold 的消费方；MAG 保留 grant-domain source、descriptor、schema、sidecar、projection builder、receipt writer 和 focused tests，不持有通用 scaffold template owner。剩余功能/结构缺口是 OPL 抽取并生产消费通用 template/checklist、MAG generated surface owner layout cutover，以及 legacy runtime/journal/probe/compat residue 的 no-active-caller physical cleanup。

当前功能/结构差距：OPL template/checklist、generated surface production consumption 和 no-active-caller cleanup 都未完成；no-forbidden-write、direct/hosted parity、restore/provenance 只作为测试/证据差距。
...
目标：在 generated surface production consumption、active caller cutover、memory/package/lifecycle handoff、App/workbench shell 和 skeleton cleanup 完成后，用 focused hosted attempt 和 continuous read snapshot 验证 MAG owner receipt / typed blocker / no-regression path。
...
1. 在 OPL 侧补齐 pack compiler / generated surface，把 MAG 当前手写 product status、user-loop、sidecar、grouped CLI/API、projection、lifecycle wrapper 的输入迁到 declarative grant pack，并执行 active caller cutover。
2. 在 OPL/App 侧补齐 generic primitive replacement 的生产消费：workspace/source intake shell、memory locator/writeback transport、package/export lifecycle shell、quality/readiness projection shell、operator observability/SLO、App/workbench drilldown 和 scaffold/template checklist。
3. 按 latest owner surface 推进 MAG legacy physical cleanup；legacy runtime/journal/probe/compat residue、旧模块、旧接口、旧测试或 hand-written thin shell 已无 active caller 就直接清理或归档。
4. 用真实 workspace 把 grant strategy memory accepted/rejected receipts 推进到 memory body migration 与 retrieval/writeback apply 泛化，并让 OPL/App 消费 `memory-receipt-projection` 的 body-free refs。
5. 用真实 workspace 跑 package/export lifecycle 与 cleanup/restore/retention guarded receipts，并让 OPL/App 消费 `package-lifecycle-handoff` 与 `lifecycle-receipt-bundle`。
...
7. 最后扩展到 long-run live soak；live soak 是生产验收，不再作为功能迁移、上收、清理的前置条件。

完成上述步骤后，MAG 才能从 “OPL-compatible domain authority pack + migration-bridge thin surface prepared” 进入更接近 “production-hosted declarative grant agent” 的状态。
