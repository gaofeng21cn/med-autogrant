...
2026-07-07 evidence read-model wrapper thinning：controlled-soak observability、stage-attempt observability、OPL conflict envelope、hosted receipt verification 与 external evidence ledger 已从 `ProductEntryEvidenceMixin` pass-through surface 收回 leaf module；这减少测试保活的第二入口，不改变 owner receipt signer、CLI authority command、domain handler closeout 或 retained big-shell physical-delete gate。
