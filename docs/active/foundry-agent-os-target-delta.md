...
本文只回答 MAG 在 OPL family `Foundry Agent OS` 目标形态下“哪些上收到 OPL，哪些保留为 MAG authority kernel”。当前摘要、证据门和下一轮执行入口回到 [MAG Active Truth](./mag-ideal-state-cross-repo-gap-plan.md)；private-surface 分类从 root contracts 读取，[私有面 owner map](./opl-private-implementation-migration-inventory.md) 只提供人读导航。
