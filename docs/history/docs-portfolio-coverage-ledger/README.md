...
本目录只保留 MAG docs portfolio、private-surface、runtime/entrypoint retirement 和 no-resurrection 的压缩 provenance。历史过程按主题保留，不维护逐日 closeout 长清单。若某条历史结论仍有当前规则价值，先折回 `docs/docs_portfolio_consolidation.md`、核心五件套、Active Truth、private-surface machine owner、spec lifecycle map、contract/schema/source/test owner 或 runtime receipt，再把过程记录压缩在本目录。

这些材料不能作为 MAG 当前 Active Truth、runtime owner、grant readiness、submission readiness、production readiness、App/workbench consumption、owner delete authorization、keep-as-authority-adapter authorization 或 physical-delete authority。

## Single Source Of Truth

| Theme | Current owner |
| --- | --- |
| 当前摘要、外部 evidence gaps、下一轮 prompt | `docs/active/mag-ideal-state-cross-repo-gap-plan.md` |
| private implementation / physical morphology machine truth | root private-surface contracts；`docs/active/opl-private-implementation-migration-inventory.md` 只提供 owner 导航 |
...
| Default caller / physical delete / evidence gates | 当前读法回到 Active Truth、private-surface machine contracts、Foundry series policy、product-entry manifest、source/tests、runtime receipts 和 OPL read-model。Observed prerequisites do not authorize physical delete. |
...
| Active truth owners | `docs/active/mag-ideal-state-cross-repo-gap-plan.md` is the single Active Truth and external-evidence baton. `docs/active/opl-private-implementation-migration-inventory.md` is a thin machine-owner router, not a path-level inventory. `docs/active/foundry-agent-os-target-delta.md` remains support for the machine domain-kernel manifest, not an active plan. `docs/specs/specs_lifecycle_map.md` owns dense spec lifecycle; this file only keeps coverage provenance. | New private-surface findings start from root machine contracts. Only a fresh active-caller conflict, owner mismatch, forbidden effect or no-resurrection failure enters the Active Truth or retired-surface provenance. Production/default-caller evidence tails remain active work under the Active Truth, not this history ledger. |
...
| Parent OPL series goal | Active/support specs and product/runtime/delivery/source/policies thin-support bodies are covered by their directory indexes and `docs/specs/specs_lifecycle_map.md`; historical specs are covered by `docs/history/specs/README.md` for docs lifecycle purposes. This is a tranche-level foldback, not OPL family completion. | Next write scope is one precise SSOT theme, a triggered spec/history-body reopening, production/default-caller evidence tail, App/workbench sustained consumption, Temporal long-soak, or one physical-delete candidate. Do not expand this ledger or the Active Truth back into dated proof logs or path-level shell inventories. |
...
| Private inventory retired-register compression | The private-surface owner map carries no dated retirement register, path list or scanner count. | Current per-surface status stays in root private-surface contracts; `docs/active/opl-private-implementation-migration-inventory.md` only routes readers; no-resurrection itemization stays in `retired-surface-provenance.md`. |
...
| Active Truth reviewed-header cleanup | The Active Truth is a current evidence baton, not a dated historical snapshot or completion ledger. | `docs/active/mag-ideal-state-cross-repo-gap-plan.md` remains the only Active Truth; this ledger keeps only provenance. |
| Status stage-progress evidence compression | Live stage progress, Stage Folder / progress contracts and retired residue are summarized as current readouts without prose-freezing payload fields, file mappings or retired receipt shapes. | Declarative stage source stays in `agent/stages/manifest.json`; runtime projection resolves through `opl_generated:product_entry_manifest#/family_stage_control_plane/stages`; live truth remains in `contracts/live_stage_run_progress_evidence.json`, production acceptance contracts, stage-native artifacts and owner receipts. |
| Source-morphology guard trace compression | Dated strict-source-purity increments were folded into machine source-morphology guards and focused tests. | Human router: `docs/active/opl-private-implementation-migration-inventory.md`; machine truth: `contracts/private_functional_surface_policy.json#/physical_source_morphology_policy`, `contracts/source_closure_audit.json`, source/tests and fresh OPL readback. |
