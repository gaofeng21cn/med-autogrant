...
| Retired command / module / facade / helper / test tails | `retired-surface-provenance.md` 保留 no-resurrection rules 和 current owner refs。Old runtime command family、helper shims、private re-export facade、dynamic export lists、test star facades、package root facades、Sentrux runtime facade、patch bridge、legacy flat aliases 和 compatibility aggregate tests 已压缩。 |
...
- Scope reviewed: root `README*`, `docs/*.md`, `docs/active/*.md`, `docs/public/*.md`, `docs/product/*.md`, `docs/runtime/*.md`, `docs/delivery/*.md`, `docs/source/*.md`, `docs/policies/*.md`, `docs/specs/*.md`, `docs/references/**/*.md`, `docs/history/docs-portfolio-coverage-ledger/*.md`, and current private-surface / Foundry-series machine surfaces relevant to docs truth.
- SSOT decision: `docs/active/mag-ideal-state-cross-repo-gap-plan.md` remains the only active gap / completion owner; `docs/active/opl-private-implementation-migration-inventory.md` remains the path-level private implementation owner; `docs/specs/specs_lifecycle_map.md` owns dense spec lifecycle; this file only keeps coverage provenance.
- Content-level consolidation: prior active-shell SSOT, Foundry command-surface field retirement, authoring-route history, foundation history, post-R5A local-runtime history entries, product/runtime/delivery/source/policies thin-support indexes, specs lifecycle map, and history specs index were compressed into the provenance groups above. Detailed deletion / field-retirement facts remain recoverable from git history and `retired-surface-provenance.md`; current rules remain in owner docs and machine surfaces.
- Retired / guarded: no source, contract, test, workflow, package or CLI/API file changed in this tranche. Guarded retired surfaces remain `run-local`, `runtime-run`, `runtime-resume`, `probe-upstream-hermes`, active `frontdoor` fields/help wording, local journal / attempt ledger, Gateway/local-manager default path, flat aliases, facades, patch bridge, compatibility aggregate tests, and physical-delete-by-read-model claims.
- Remaining MAG scope under the parent OPL series goal: private inventory detail refresh, concrete source/test/workflow/package retirement candidates, and production/default-caller evidence tails. Active/support specs and product/runtime/delivery/source/policies thin-support bodies are covered by their directory indexes and `docs/specs/specs_lifecycle_map.md`; historical specs are covered by `docs/history/specs/README.md` unless a fresh scan finds an active-looking checklist, reusable prompt, current owner conflict, or machine surface that still depends on an uncompressed historical rule.
- Next write scope: after fresh intake, choose one precise SSOT theme, a triggered spec/history-body reopening, or one physical-delete candidate. Do not expand this ledger or the active gap plan back into dated proof logs or path-level shell inventories.
