# MAG docs portfolio coverage ledger archive

Owner: `Med Auto Grant`
Purpose: `docs_portfolio_coverage_ledger_archive`
State: `history_provenance`
Machine boundary: 本文是人读历史 coverage ledger。当前机器真相继续归 `contracts/runtime-program/current-program.json`、schemas、source、CLI/API 行为、runtime receipts 和语义化 `human_doc:*` id。

本文归档原 `docs/docs_portfolio_consolidation.md` 中的 dated coverage ledger。它只记录当时的 frozen inventory、审计范围、证据读法和 foldback 过程；MAG 当前 docs lifecycle truth 以 `docs/docs_portfolio_consolidation.md`、核心五件套、active gap plan、contracts/source/runtime evidence 为准。
