...
| Theme | Current owner |
| --- | --- |
| MAG Active Truth / external evidence tail | `docs/active/mag-ideal-state-cross-repo-gap-plan.md` |
| private-surface machine truth | root private-surface contracts；`docs/active/opl-private-implementation-migration-inventory.md` 只提供 owner 导航 |
...
| product-entry/package support | `docs/specs/product-entry-support-record.md`, direct domain handler, package authority source/tests, declarative stage manifest |
...
| package/export authority | MAG package authority source, declarative stage manifest, package/export gates, owner receipts, typed blockers |
...
| --- | --- | --- |
| old public/domain-entry runtime commands: `run-local`, `runtime-run`, `runtime-resume`, `probe-upstream-hermes` | grouped public CLI catalog, service-safe domain command catalog, direct domain handler/CLI tests, current-program runtime owner fields, active plan no-resurrection tail, and the runtime-first R1-R5 compression record in `docs/history/specs/README.md` | Do not restore flat runtime commands, deleted `mainline_status.py` / `functional_closure_skeleton.py`, local journal owner, attempt ledger, Hermes probe, Gateway/local-manager default path, hosted runtime claim, or compatibility public command aliases. |
| Foundry Agent active `frontdoor` projection fields and help wording: `status.public_frontdoor`, `status.executable_frontdoors`, `interfaces.ordinary_public_frontdoor_spine`, `validation.checked_frontdoor_operations`, `missing_frontdoor_operation:*`, and executable frontdoor help text | `contracts/foundry_agent_series.json`, `src/med_autogrant/cli.py`, grouped public command routing, root README quickstart, `docs/architecture.md`, and `tests/test_cli_smoke.py` | Do not restore deleted `foundry_series_cli.py` or active CLI JSON/help/test/status wording under `frontdoor`. Historical `frontdoor` text may remain only in history/provenance or no-resurrection/negative-guard context. |
...
| `product-entry` / `domain_runtime` stale aggregate exports and `domain_runtime_parts.patch_targets` patch bridge | source tree, `substrate.__all__`, structure tests, CLI/API behavior, runtime receipts, private-surface machine contracts | Do not restore aggregate re-export, patch bridge, facade target, or compatibility-only tests after active caller migration. |
...
| default caller deletion as automatic physical-delete authority | Active Truth, private-surface machine contracts, Foundry series policy, current-program, OPL read-model, MAG owner delete / keep-as-authority-adapter / typed blocker receipt | Observed default caller prerequisites do not authorize physical delete. Require explicit MAG owner receipt and all no-active-caller / parity / no-forbidden-write gates. |
| no-rewrite / SSOT closeout files as current truth | core docs, Active Truth, private-surface machine contracts, specs lifecycle map, source/tests/CLI/API/runtime receipts | Do not link per-tranche closeouts as current evidence. Durable conclusions must live in current owners; this file keeps compressed provenance. |
