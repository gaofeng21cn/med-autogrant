...
| --- | --- | --- |
| old public/domain-entry runtime commands: `run-local`, `runtime-run`, `runtime-resume`, `probe-upstream-hermes` | grouped public CLI catalog, service-safe domain command catalog, current-program runtime owner fields, negative tests, active plan no-resurrection tail | Do not restore flat runtime commands, local journal owner, attempt ledger, Hermes probe, Gateway/local-manager default path, or compatibility public command aliases. |
