...
| --- | --- | --- |
| old public/domain-entry runtime commands: `run-local`, `runtime-run`, `runtime-resume`, `probe-upstream-hermes` | grouped public CLI catalog, service-safe domain command catalog, `src/med_autogrant/mainline_status.py`, `src/med_autogrant/product_entry_parts/functional_closure_skeleton.py`, `tests/test_domain_entry.py`, current-program runtime owner fields, and active plan no-resurrection tail | Do not restore flat runtime commands, local journal owner, attempt ledger, Hermes probe, Gateway/local-manager default path, or compatibility public command aliases. |
