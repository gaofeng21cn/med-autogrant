...
Purpose: `historical_r4a_final_freeze_export_activation_provenance`
State: `history`
Machine boundary: 本文是人读历史 spec，保留 2026-04-09 R4.A final freeze and export package activation package 的形成过程。当前 package lifecycle shell 与 generic artifact handling 归 OPL/shared family layer，MAG 保留 grant truth、package authority、fundability/quality/export verdict 与 owner receipt；当前 export/package behavior 以核心五件套、active gap plan、`docs/specs/README.md`、`docs/specs/specs_lifecycle_map.md`、contracts、schemas、source、CLI/API 行为与 `contracts/runtime-program/current-program.json` 为准。
