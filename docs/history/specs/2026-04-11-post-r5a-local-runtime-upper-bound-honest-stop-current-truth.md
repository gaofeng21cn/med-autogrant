...
Purpose: `historical_local_runtime_honest_stop_provenance`
State: `history`
Machine boundary: 本文是人读历史 spec，保留 post-R5A local runtime upper-bound / honest-stop 形成过程。当前 default runtime owner、hosted provider、local runtime residue、package/export behavior 与机器行为以核心五件套、`docs/specs/README.md`、`docs/specs/specs_lifecycle_map.md`、contracts、schemas、source、CLI/API 行为与 `contracts/runtime-program/current-program.json` 为准。
