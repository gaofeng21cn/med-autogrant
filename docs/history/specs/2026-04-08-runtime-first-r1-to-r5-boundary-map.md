...
Purpose: `historical_runtime_first_boundary_map_provenance`
State: `history`
Machine boundary: 本文是人读历史 spec，保留 2026-04-08 runtime-first R1-to-R5 boundary map、local runtime ladder 与 honest-stop guard 的形成过程。当前 runtime owner 是 OPL/Temporal，MAG 不持有 generic daemon、scheduler、attempt loop 或 attempt ledger；R-series local runtime ladder、journal、Gateway、host-agent 与 hostedization wording 只作 provenance。当前机器行为以核心五件套、active gap plan、`docs/specs/README.md`、`docs/specs/specs_lifecycle_map.md`、contracts、schemas、source、CLI/API 行为与 `contracts/runtime-program/current-program.json` 为准。
