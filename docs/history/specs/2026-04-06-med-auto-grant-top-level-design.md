...
Purpose: `historical_foundation_top_level_design_provenance`
State: `history`
Machine boundary: 本文是人读历史 spec，保留 2026-04-06 顶层设计、命名、MVP 与方法论形成过程。当前 MAG role、runtime owner、OPL/Temporal boundary、active specs 与机器行为以核心五件套、`docs/specs/README.md`、`docs/specs/specs_lifecycle_map.md`、contracts、schemas、source、CLI/API 行为与 `contracts/runtime-program/current-program.json` 为准。
...
当前规则是：`docs/**` 作为内部开发与维护参考，只维护中文 canonical 内容；稳定路径优先使用无语言后缀 `.md`。根层 `README*` 是否继续保留公开双语入口，由 public/product 需求单独判断，不要求 `docs/**` 维护双语镜像。
