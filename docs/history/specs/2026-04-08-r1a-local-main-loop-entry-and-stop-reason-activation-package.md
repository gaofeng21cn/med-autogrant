...
Purpose: `historical_local_main_loop_stop_reason_activation_provenance`
State: `history`
Machine boundary: 本文是人读历史 spec，保留 2026-04-08 R1.A local main loop、stop reason、runtime-run/resume 与 run journal activation package 的形成过程。当前 task runtime owner 是 OPL/Temporal；MAG 不持有 daemon、scheduler、attempt loop 或 attempt ledger，当前 runtime owner、stage executor、route truth 与机器行为以核心五件套、active gap plan、`docs/specs/README.md`、`docs/specs/specs_lifecycle_map.md`、contracts、schemas、source、CLI/API 行为与 `contracts/runtime-program/current-program.json` 为准。
