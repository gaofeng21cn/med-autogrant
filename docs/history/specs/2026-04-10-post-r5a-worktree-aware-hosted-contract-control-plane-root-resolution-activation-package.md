...
Purpose: `historical_post_r5a_worktree_aware_hosted_contract_root_resolution_provenance`
State: `history`
Machine boundary: 本文是人读历史 spec，保留 2026-04-10 worktree-aware hosted-contract control-plane root resolution activation package 的形成过程。当前 hosted/default task runtime owner 是 OPL/Temporal；hosted-contract bundle、worktree root resolution、Gateway 与 hostedization wording 只作 provenance。当前 contract export、product-entry、runtime owner 与机器行为以核心五件套、active gap plan、`docs/specs/README.md`、`docs/specs/specs_lifecycle_map.md`、contracts、schemas、source、CLI/API 行为与 `contracts/runtime-program/current-program.json` 为准。
