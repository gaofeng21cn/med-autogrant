...
Purpose: `historical_hosted_contract_bundle_route_catalog_provenance`
State: `history`
Machine boundary: 本文是人读历史 spec，保留 hosted contract bundle、route catalog 与 `MedAutoGrantDomainEntry` contract export 的形成过程。当前 contract export、route catalog、schema registry、OPL provider/App readiness 与机器行为以核心五件套、`docs/specs/README.md`、`docs/specs/specs_lifecycle_map.md`、contracts、schemas、source、CLI/API 行为与 `contracts/runtime-program/current-program.json` 为准。
