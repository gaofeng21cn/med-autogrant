...
Purpose: `historical_second_grant_family_onboarding_activation_provenance`
State: `history`
Machine boundary: 本文是人读历史 spec，保留 2026-04-08 P5.A 第二 grant family onboarding activation package 的预冻结边界。当前 admitted domain、family expansion、grant pack admission、OPL generated/hosted surface 与 federation 前置条件以核心五件套、active gap plan、`docs/specs/README.md`、`docs/specs/specs_lifecycle_map.md`、contracts、schemas、source、CLI/API 行为与 `contracts/runtime-program/current-program.json` 为准；本文不是 active P5 backlog、second-family admitted claim 或 federation-ready claim。
