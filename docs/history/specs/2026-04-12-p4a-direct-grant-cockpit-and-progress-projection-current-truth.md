...
Purpose: `direct_grant_progress_cockpit_projection_support_record`
State: `support_current_truth`
Machine boundary: 本文是人读支撑记录，只在 direct grant progress / cockpit projection subsection 内有效。机器真相继续归 source、schemas、product-entry manifest、contracts 和 `contracts/runtime-program/current-program.json`。
