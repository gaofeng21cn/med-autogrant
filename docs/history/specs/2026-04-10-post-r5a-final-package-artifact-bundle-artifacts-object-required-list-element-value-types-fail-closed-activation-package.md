# Post-R5A Final Package Artifact Bundle Artifacts Object Required List Element Value Types Fail-Closed Activation Package
...
- Active slice: `build-final-package artifact bundle artifacts object required list element value types fail-closed hardening`
- Status: `frozen / ready for implementation`
- Upstream prerequisite:
  - `R1.A / R1.B / R2.A / R3.A / R4.A / R5.A` 已 absorbed
  - post-`R5.A` walkthrough / output-consistency / worktree-aware hosted-contract / local-runtime validation-failed / final-package malformed artifact bundle / hosted-contract malformed final-package / hosted-contract final-package required scalar-field / hosted-contract final-package required nested-field / hosted-contract final-package checkpoint-semantics / hosted-contract final-package lineage value-type / hosted-contract final-package freeze_manifest value-type / final-package artifact-bundle required nested-field / final-package artifact-bundle required scalar value-type / final-package artifact-bundle summary count value-type / final-package artifact-bundle artifacts list value-type / final-package artifact-bundle artifacts object value-type / final-package artifact-bundle artifacts object primary-id field / final-package artifact-bundle artifacts list element-shape / final-package artifact-bundle artifacts object linkage-id field / final-package artifact-bundle artifacts object required string-field / final-package artifact-bundle artifacts object required list-field / final-package artifact-bundle artifacts list element required string value-type / final-package artifact-bundle artifacts list element linked_object_ids field value-type / final-package artifact-bundle artifacts list element linked_object_ids list element value-type slices 已 absorbed
  - `build-final-package` 继续只属于本地 finalization / export boundary

## Goal

在不改写 `build-final-package` object boundary、不新增 runtime command、也不打开 actual hosted runtime / `P5` / 新 formal entry 的前提下，把 artifact object required list fields 的 list element value-type drift 收紧成 repo-tracked fail-closed contract：

- 当以下 list element 不是非空字符串时，`build-final-package` 必须稳定抛出 `WorkspaceStateError`：
  - `artifacts.selected_direction.required_evidence_ids[0]`
  - `artifacts.selected_question.subquestions[0]`
  - `artifacts.selected_question.linked_evidence_ids[0]`
  - `artifacts.argument_chain.linked_evidence_ids[0]`
  - `artifacts.fit_mapping.linked_evidence_ids[0]`
- 不得再出现：
  - `artifacts.selected_direction.required_evidence_ids[0]=None` 却仍成功产出 final package
  - `artifacts.selected_direction.required_evidence_ids[0]={}` 却仍成功产出 final package
  - `artifacts.selected_direction.required_evidence_ids[0]=""` 却仍成功产出 final package
  - `artifacts.selected_question.subquestions[0]=None` 却仍成功产出 final package
  - `artifacts.selected_question.linked_evidence_ids[0]={}` 却仍成功产出 final package
  - `artifacts.argument_chain.linked_evidence_ids[0]=""` 却仍成功产出 final package
  - `artifacts.fit_mapping.linked_evidence_ids[0]=None` 却仍成功产出 final package
...
- `_read_artifact_bundle(...)` 已开始校验：
  ...
  - 上述字段自身都已经被收紧为 `list`
- 但仍未显式校验这些列表内部元素是否真的是非空字符串

这导致多个 concrete gap：

1. `artifacts.selected_direction.required_evidence_ids[0]=None` 时，CLI 仍会成功产出 final package；
2. `artifacts.selected_direction.required_evidence_ids[0]={}` 时，CLI 仍会成功产出 final package；
3. `artifacts.selected_direction.required_evidence_ids[0]=""` 时，CLI 仍会成功产出 final package；
4. `artifacts.selected_question.subquestions[0]=None` 时，CLI 仍会成功产出 final package；
5. `artifacts.selected_question.linked_evidence_ids[0]={}` 时，CLI 仍会成功产出 final package；
6. `artifacts.argument_chain.linked_evidence_ids[0]=""` 时，CLI 仍会成功产出 final package；
7. `artifacts.fit_mapping.linked_evidence_ids[0]=None` 时，CLI 仍会成功产出 final package。

## In Scope

### 1. artifact object required list element value validation
...
- 不得继续成功产出 final package 掩盖 required list element drift
...
4. 构造非法 required list element value-types，执行：
   - `PYTHONPATH=src python3 -m med_autogrant build-final-package ... --format json`
   - 断言 `None / {} / ""` 输入 fail-closed，而不是成功产出 final package
...
- `build-final-package` 继续只消费上述 required list fields 的非空字符串 list elements，不放宽 `R2.A -> R4.A` 输入 contract
- malformed artifact bundle 只能 fail-closed，不能 fallback、不能补默认值掩盖 required list element drift
