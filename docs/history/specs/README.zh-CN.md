...
本索引是旧技术 specs 与 tranche 记录的历史入口。大多数源文件仍保留在 `docs/specs/`，这样旧审计链接、current-program 指针和历史绝对路径说明仍可读。

阅读旧 program 背景时从这里进入。当前真相请回到 [Specs 索引](../../specs/README.zh-CN.md)、核心五件套与 `current-program.json`。
...
- 2026-04-11 Hermes/reset 与 local-runtime closeout 记录：`../../specs/2026-04-11-*.md`；这些是 provider/proof 或 honest-stop provenance，不是当前默认 runtime owner。
- 2026-04-12 hosted-caller、OPL alignment、product-entry 与 handoff 记录：`../../specs/2026-04-12-*.md`；hosted contract 与 caller consumption 细节仍可作为支持材料，旧 gateway/federation/provider 说法按历史处理。
