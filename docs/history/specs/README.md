...
| Runtime-first、local-runtime、Gateway/federation 与 provider proof | R/P/post-R5A runtime-productization packages、local-runtime hardening、honest-stop、Gateway/federation future activation、Hermes-backed proof 和 upstream Hermes reset records 的压缩 provenance。 | 只作为 retired provider proof / migration lesson。默认 runtime owner、provider-backed substrate、queue、attempt ledger、App/workbench 和 executor adapter 边界回到 OPL/core docs、active gap plan、contracts/source/tests。 |
| Package/export、hosted contract bundle 与 fail-closed hardening | final-package artifact bundle、hosted contract bundle、route checkpoint shape、workspace validator 和 package/export fail-closed hardening provenance 的压缩记录。 | 当前 package/export authority、schema behavior 和 fail-closed gates 回到 package/export source、schemas、tests、support record 与 owner receipts；不恢复旧 hosted runtime 或 Gateway route。 |
...
- author-side route / executor envelope / pending handoff 历史 specs 已压缩为本主题索引和 coverage ledger 记录；原 4 个逐阶段 route/handoff 历史文件已删除。当前 route truth 回到 active specs、`../../specs/product-entry-support-record.md`、`src/med_autogrant/domain_runtime_parts/contracts.py`、`src/med_autogrant/route_report.py`、`schemas/v1/executor-routing-contract.schema.json`、direct domain-handler tests、核心五件套和 `current-program.json`。已退役的 `product_entry.py` 与旧 aggregate tests 只从 git history读取。
...
- 2026-04-06 MAG foundation / NSFC main flow / top-level design 历史 specs 已压缩为本主题索引和 coverage ledger 记录；原 2 个早期 foundation 历史文件已删除。当前 truth 回到 `contracts/runtime-program/current-program.json`、核心五件套、`../../references/med-auto-grant-ideal-state.md`、`../../active/mag-ideal-state-cross-repo-gap-plan.md`、`../../specs/README.md`、`../../specs/specs_lifecycle_map.md`、`agent/` Declarative Grant Pack、canonical stage manifest、OPL generated stage plane、quality gates、source/contracts/tests 与 MAG-owned owner receipt / typed blocker / package authority surfaces。
...
- runtime-first R1-R5 boundary map、R1.A / R2.A / R3.A / R4.A / R5.A activation package、runtime-first productization program 和 R3.A machine-applicable revision mutation contract 历史 specs 已压缩为本主题索引和 coverage ledger 记录；原逐 slice / program / contract 长正文已删除。当前 truth 回到 `contracts/runtime-program/current-program.json`、核心五件套、`../../active/mag-ideal-state-cross-repo-gap-plan.md`、`../../specs/README.md`、`../../specs/specs_lifecycle_map.md`、active current specs、revision/package/export source、schemas、tests、product-entry manifest 和 owner receipts。
...
- post-R5A local-runtime / worktree-root / route-checkpoint / local runtime hardening / honest-stop 历史 specs 已压缩为本主题索引和 coverage ledger 记录；原逐 slice、hardening brief 和 honest-stop 长正文已删除。当前 truth 回到 `contracts/runtime-program/current-program.json`、`src/med_autogrant/route_report.py`、`src/med_autogrant/control_plane.py`、`src/med_autogrant/domain_runtime_parts/contracts.py`、direct domain-handler/CLI tests、核心五件套、active gap plan、runtime support index 和 active/support specs lifecycle map。已删除的 `functional_closure_skeleton.py`、`mainline_status.py` 与旧 hosted-control aggregate tests 不再是 current owner。
...
- future P5 second-family onboarding / federation activation package 历史 specs 已压缩为本主题索引和 coverage ledger 记录；原 P5.A / P5.B 长正文已删除。当前 family admission、federation、Gateway wording、OPL generated/hosted surface 和 domain admission truth 回到核心五件套、OPL owner docs、active gap plan、`../../specs/README.md`、`../../specs/specs_lifecycle_map.md`、contracts/source/tests 和 current domain-admission surfaces。
- 该压缩只退役 P5.A second grant family onboarding 与 P5.B federation contract freeze 的 historical activation checklist；不声明 second family admitted、federation-ready、Gateway-ready、cross-domain runtime owner、web runtime、same-repo HITL、public MCP/controller formal entry 或 submission-grade autopilot。
- `2026-04-13-hermes-native-critique-proof-tombstone.md` 继续保留为 path-stable tombstone evidence，因为 current contracts/source/tests 仍引用该路径；该保留不把 Hermes-native proof 提升为 current provider owner 或 default executor。
