...
Purpose: `historical_stage_action_executor_envelope_activation_provenance`
State: `history`
Machine boundary: 本文是人读历史 spec，保留 2026-04-08 R1.B stage action executor envelope activation package 的形成过程。当前 task runtime owner 是 OPL/Temporal；MAG 不持有 generic queue、attempt loop、state-machine runner 或 attempt ledger，当前 stage executor、route continuation、runtime owner 与机器行为以核心五件套、active gap plan、`docs/specs/README.md`、`docs/specs/specs_lifecycle_map.md`、contracts、schemas、source、CLI/API 行为与 `contracts/runtime-program/current-program.json` 为准。
