...
- Stage Pack v2 只要求 mutating action 的 `stage_route` 精确覆盖 manifest 中声明该 action 的 Stage，并按 `next_stage_refs` 排序；read-only action 不声明执行 route，观察范围继续由 `allowed_action_refs` 表达。
