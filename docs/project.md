...
- 在 OPL 目标形态中只把 MAG projection 注册给 `OPL Runtime Manager`；Runtime Manager 可以管理 provider-backed family runtime 与状态索引，但不得替代 MAG author-side truth、quality gate、submission-ready export gate 或 route-selected executor。
