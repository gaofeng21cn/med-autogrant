...
当前活跃计划 / evidence ledger：

- [MAG 理想目标态差距与完善计划](./mag-ideal-state-cross-repo-gap-plan.zh-CN.md)：MAG repo closure 已由 `product-entry-manifest.ideal_state_closure_status` 承载；剩余 gate 是 production live soak、no-regression evidence、memory apply 泛化、OPL generic primitive absorption，以及 legacy direct-retirement audit evidence。
