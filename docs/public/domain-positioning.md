...
Machine boundary: 本文是人读公开定位补充。产品与机器真相继续归核心文档、contracts、schemas、source、CLI/API 行为、product-entry manifest、runtime receipts 与 grant workspace artifacts。
...
- `product entry/product status/direct-entry/user-loop`：单一 app skill 下的内部 command contract / direct-product projection
...
- product-entry、product status、direct-entry、user-loop 继续位于 app skill 之下，不作为对外第一主语
