...
`mag_consumer_thinning_contract` 进一步把 OPL handoff boundary 机器化：MAG sidecar/direct path 保持 thin adapter，OPL 才是 generic primitive replacement owner。MAG 当前可交付的是合同、refs、receipt writer、typed blocker 和 no-forbidden-write guard；真实 OPL replacement consumption、workspace memory body migration、package/lifecycle workspace receipt、focused hosted receipt verification 和 live soak 是后续 evidence gate。
