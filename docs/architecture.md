...
`agent/` 持有 stages、prompts、skills、knowledge 与 quality gates。`agent/stages/manifest.json`、`contracts/pack_compiler_input.json`、`action_catalog.json` 和 `functional_privatization_audit.json` 是 OPL 可消费的机器输入；`family_stage_control_plane` 由 OPL Pack生成，不作为 MAG tracked contract。
