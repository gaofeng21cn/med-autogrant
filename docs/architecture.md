...
- `product-entry-manifest.mag_consumer_thinning_contract.external_evidence_request_pack` 是 OPL/App/production 证据门的请求面；具体 request refs、forbidden payload classes 与关闭条件以机器合同、active gap plan 和 private inventory 为准。External evidence ledger 和 grant-stage controlled attempt closeout refs 只作为 MAG closeout / OPL preflight 消费证据，不能作为 grant-ready、fundability-ready、quality/export-ready 或 submission-ready 成功证明。
...
- `execute-critique-revision-loop` 在现有 `execute-critique-pass` 与 `execute-revision-pass` 之上复用同一份 route truth；它不会改写单步 pass 语义。该 repo-local controller 只接受 OPL default stage attempt envelope，并在缺少 `one-person-lab` owner、`codex_cli` executor 或 OPL attempt lease/receipt 时 typed-blocker fail-closed。
- `execute-authoring-mainline-loop` 在 `determine_next_step(...)` 与单步 pass builder 之上进一步向上扩，把 rollback 到 `direction_screening / question_refinement / argument_building / fit_alignment` 的重建纳入 bounded controller authority target；它同样只接受 OPL default stage attempt envelope，不作为 MAG-owned durable mainline loop。
...
- `execute-grant-autonomy-controller` 位于已有 discovery / selector / initializer / mainline loop / quality surface 之上，负责预算、轮次、blocker 队列、evidence gap 队列、同一基金任务内的 rollback/continue 决策记录与 fail-closed 报告，不替代下游单步 pass，也不持有 durable loop。该入口缺少 OPL default stage attempt lease / receipt 时返回 `typed-blocker:mag/autonomy-controller/opl-default-stage-attempt-required` 语义的 typed blocker。
- 同一条 `pass critique` 现在也支持显式 `executor_kind=hermes_agent`：这条 receipt lane 会通过 OPL `executor run` 进入 `one-person-lab` 持有的 Agent Executor Adapter；MAG 只提供 OPL 可调用的 domain closeout helper，并消费 OPL `AgentExecutionReceipt` 与 `mag_critique_closeout_packet`。helper 内部会读取本机 `~/.hermes/config.yaml` 的 model/provider/base_url/api_mode/reasoning_effort 并调用上游 Hermes full agent loop；只有在完成整轮 loop、拿到真实工具事件、合法 domain closeout、`receipt_owner=one-person-lab` 且 `mag_executor_owner=false` 的 OPL receipt 时才通过，否则 fail-closed。
