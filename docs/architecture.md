...
`mag_consumer_thinning_contract` 进一步把 OPL handoff boundary 机器化：MAG sidecar/direct path 保持 thin adapter，OPL 才是 generic primitive replacement owner。MAG 当前可交付的是合同、refs、receipt writer、typed blocker、no-forbidden-write guard、focused hosted receipt verification、lifecycle receipt bundle、memory receipt read projection、package lifecycle handoff projection 和 continuous receipt reconciliation snapshot；真实 OPL replacement production consumption、workspace memory body migration、package/lifecycle workspace receipt 泛化和 live soak 是后续 evidence gate。

`mag_consumer_thinning_contract.privatized_functional_module_audit` 是当前 OPL 统一审计读取的功能私有化 closeout 面。它把 runtime registration、task lifecycle、session ledger / attention queue、lifecycle adapter、observability、sidecar/product status、package lifecycle、source intake、人用 workbench / scheduler / daemon 归到 OPL-owned generic primitive consumer；把 grant lifecycle stage、fundability / quality / export verdict、package readiness / submission-ready、grant transition oracle、owner receipt 和 grant strategy memory accept/reject 归到 MAG-owned grant authority；把旧 runtime owner、patch bridge、compat aggregate test、legacy shell alias 和 repo-owned scheduler/daemon 归到 retire/tombstone。这个审计面用于防止误读：MAG 需要收薄通用运行外围，但不能删除或外包 grant lifecycle、package readiness、fundability、quality、export verdict 或 owner receipt。

该 audit item 现在同时列出代码路径、活跃调用者、调用状态、迁移动作、保留理由和不能上收原因。架构读法是：MAG 当前仍有本地 journal / attempt ledger、controller/loop、workspace/source validation、memory/package/status/sidecar/CLI wrapper 与 optional `Hermes-Agent` proof state-db 相关代码；这些 active path 只允许作为 domain thin adapter、local diagnostic、refs-only projection 或 explicit proof lane 存在。OPL 应上收通用 ledger、queue、lifecycle、source intake、artifact/package lifecycle、observability、workbench、scheduler/daemon 和 generic runner；MAG 不能外包 grant truth、memory body、fundability/quality/export verdict、submission-ready package authority、grant transition oracle 或 owner receipt。
...
- `runtime run` / `runtime resume` 直接通过 `MedAutoGrantDomainEntry -> MagDomainRuntime` 落到 MAG-owned runtime loop。
