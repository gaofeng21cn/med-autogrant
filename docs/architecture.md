...
`agent/` 持有 stages、prompts、skills、knowledge 与 quality gates。`contracts/pack_compiler_input.json`、`action_catalog.json`、`stage_control_plane.json` 和 `functional_privatization_audit.json` 是 OPL 可消费的机器输入。
