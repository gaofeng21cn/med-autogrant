...
MAG 侧当前实现的是 product sidecar adapter，而不是长期常驻 sidecar daemon。`product sidecar export --input <workspace.json> --format json` 导出 runtime_control、runtime_continuity、TODO/explicit wakeup、autonomy-controller、domain-memory、owner receipt、lifecycle receipt 与 user-loop attention queue；其中 TODO/wakeup 只把 OPL typed queue 指向 `open_grant_user_loop` / `product user-loop` 这个 MAG domain action target，不再把 `status/read`、`user-loop/wakeup`、`notification/receipt`、`autonomy-controller/dry-run` 或 `autonomy-controller/guarded-run` 作为 sidecar dispatch action。`product sidecar dispatch --task <task.json> --format json` 只允许 MAG-owned receipt / refs-only domain actions：`domain-memory/propose`、`domain-memory/decide`、`stage-attempt/closeout`、`lifecycle/receipt`、`closeout/codex-stage-receipts`、`closeout/operator-readiness`、`closeout/physical-morphology-guard` 与 `closeout/executor-first-bundle`。autonomy-controller 的 domain authority 仍由 `MedAutoGrantDomainEntry` 与 CLI direct path 承载，不经 product sidecar generic wrapper。
...
- `product-entry-manifest.mag_consumer_thinning_contract.external_evidence_request_pack` 是 OPL/App/production 证据门的请求面：它要求外部 caller 返回 generated/hosted caller consumption、workbench refs consumption、release/dist consumption、owner receipt / typed blocker roundtrip、no-forbidden-write、direct/hosted parity 和 long-soak receipt reconciliation evidence。2026-05-20 `contracts/external_evidence/mag-evidence-receipt-ledger.json` 已把 request-level accounting 闭合为 body-free verified receipt refs，并补齐 `grant_stage_controlled_attempt_closeout` 指向六个 stage 的 expected receipt / monitor freshness refs；这些 refs 只能作为 MAG closeout / OPL preflight 消费证据，不能作为 grant-ready、fundability-ready、quality/export-ready 或 submission-ready 成功证明。
...
- OPL stage-led runtime framework 只消费 `runtime_control.semantic_closure`、`skill_catalog.domain_projection.runtime_continuity`、`workspace progress / cockpit`、hosted contract bundle 与 artifact/wakeup locator；高频索引由 OPL-owned helper/index layer 持有实现，MAG 只通过 `native_helper_consumption.proof_surface` 暴露 workspace/session/artifact/TODO-wakeup/runtime-health refs-only coverage 与 `opl_index_only` 写入边界，不声明 helper 语言、二进制或 backing helper id；它不写 MAG authoring truth，也不替代 route-selected executor。
