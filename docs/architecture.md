...
- `MedAutoGrantProductEntry` 现在还提供 MAG-owned production functional closure projection：`product domain-memory-proposal` 生成 writeback proposal envelope，`product domain-memory-decision` 生成 accept/reject decision 与 operator receipt projection；`product-entry-manifest` 额外暴露 `owner_receipt_contract`、`controlled_domain_memory_apply_proof`、`lifecycle_guarded_apply_proof`、`physical_skeleton_follow_through` 与 `repo_source_layout_audit`。这些面只投影 runtime/writeback refs、owner receipt refs、lifecycle typed blockers 和 repo-source audit，不把真实 memory entry、grant artifact 或 receipt instance 写入 repo。
