...
- `OPL family runtime provider`：可由 OPL 选择和演进的外部 provider；旧 Temporal/Hermes-first 说法属于 provider-specific 或历史实现记录；Hermes-Agent / Claude Code 当前只作为显式 executor adapter backend 接入
...
- `MedAutoGrantProductEntry` 现在还提供 MAG-owned domain memory apply projection：`product domain-memory-proposal` 生成 writeback proposal envelope，`product domain-memory-decision` 生成 accept/reject decision 与 operator receipt projection；`product-entry-manifest` 额外暴露 `controlled_domain_memory_apply_proof` 与 `repo_source_layout_audit`。这些面只投影 runtime/writeback refs 和 repo-source audit，不把真实 memory entry、grant artifact 或 receipt instance 写入 repo。
