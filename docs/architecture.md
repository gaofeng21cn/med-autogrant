...
- `OPL family runtime provider`：可由 OPL 选择和演进的外部 provider；旧 Temporal/Hermes-first 说法属于 provider-specific 或历史实现记录；Hermes-Agent 当前只作为显式 executor adapter / proof lane 接入
...
- `MedAutoGrantProductEntry` 现在还提供 MAG-owned domain memory apply projection：`product domain-memory-proposal` 生成 writeback proposal envelope，`product domain-memory-decision` 生成 accept/reject decision 与 operator receipt projection。两者只投影 runtime/writeback refs，不把真实 memory entry、grant artifact 或 receipt instance 写入 repo。
