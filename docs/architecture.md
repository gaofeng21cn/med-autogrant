...
该 audit item 现在同时列出代码路径、活跃调用者、调用状态、迁移动作、保留理由和不能上收原因。架构读法是：旧 local journal / attempt ledger 与 Hermes/Gateway/local-manager runtime owner 已从 active source 删除；MAG 当前只保留 workspace/source validation、memory/package/status/domain-handler/CLI target、refs-only runtime_control、显式 executor adapter request/receipt vocabulary，以及只能在 OPL default stage attempt lease / receipt 下运行的 bounded controller authority target。repo-local Hermes helper/runtime implementation 若仍存在，只能作为 OPL adapter 迁出/删除候选，不是 MAG owner surface。`critique_loop_controller`、`authoring_mainline_controller` 与 `grant_autonomy_controller` 缺少 `one-person-lab` owner、`codex_cli` default executor 或 OPL attempt lease/receipt 时必须返回 typed blocker，不能启动 MAG-owned durable loop。OPL 应生成或托管通用 ledger、queue、lifecycle、source intake、artifact/package lifecycle、observability、workbench、scheduler/daemon、generic runner 和 executor substrate；MAG 不能外包 grant truth、memory body、fundability/quality/export verdict、submission-ready package authority、grant transition oracle 或 owner receipt。
