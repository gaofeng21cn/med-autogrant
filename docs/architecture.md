...
- 目标中的 `OPL Runtime Manager` 只消费 `runtime_control.semantic_closure`、`skill_catalog.domain_projection.runtime_continuity`、`workspace progress / cockpit`、hosted contract bundle 与 artifact/wakeup locator；高频索引可由 OPL Rust native helper 执行，但它不写 MAG authoring truth，也不替代 route-selected executor。
