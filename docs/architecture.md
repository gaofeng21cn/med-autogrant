...
`Hermes-Agent` 和 `Claude Code` 相关路径当前属于 OPL generic Agent Executor Adapter 的显式 opt-in backend，而不是默认公开 capability contract、公开第一入口或 MAG runtime owner。MAG 不管理这些 executor substrate；MAG 只声明/消费 request、receipt、closeout 和 critique payload vocabulary。非默认 executor 也只能挂在同一套 `CLI / MedAutoGrantDomainEntry / route contract / export contract` 下面。
...
`temporal_stage_run_consumption_policy` 是 `family_action_catalog`、`generated_surface_handoff`、`runtime_control` 与 `product-status` 共享投影的 StageRun 消费边界。它把 Temporal 明确归到 OPL-hosted durable substrate：MAG 可以消费 `temporal_stage_run_ref`、provider attempt / completion ref 与 stage attempt ref，但不能拥有 Temporal runtime、attempt ledger、worker/scheduler，也不能写 OPL stage attempt records。provider completion 和 generated surface ready 只能作为 runtime/projection 信号；domain completion 只能由 MAG owner receipt、typed blocker、human gate 或 route-back ref 关闭。
`agent/` 是 canonical repo-source semantic pack：`agent/prompts/` 持有六个 stage prompt，`agent/stages/` 持有 stage policy，`agent/skills/` 持有 domain skill declaration，`agent/professional_skills/` 持有 grant specialist method skills，`agent/quality_gates/` 持有 fundability、quality、export/package、memory/receipt 和 authority gate，`agent/knowledge/` 持有 grant strategy memory、package authority 和 owner receipt 知识边界。`src/` 是 domain handler、minimal authority function 和 native helper 层，不再作为 declarative stage semantics 的 canonical 位置。
...
该 audit item 现在同时列出代码路径、活跃调用者、调用状态、迁移动作、保留理由和不能上收原因。架构读法是：旧 local journal / attempt ledger 与 Hermes/Gateway/local-manager runtime owner 已从 active source 删除；workspace path/materialization/Git boundary、schema traversal、runtime registration、owner-evidence reconciliation/observability 与 artifact lifecycle envelope 已由 `opl_harness_shared` 提供，MAG 只保留 declarative profile、grant runtime constraints、receipt/typed-blocker/human-gate 与 package/export authority adapter。其余 MAG active source 只允许作为 memory/package/status/domain-handler/CLI target、refs-only runtime_control、显式 executor adapter request/receipt vocabulary，以及只能在 OPL default stage attempt lease / receipt 下运行的 bounded controller authority target。`critique_loop_controller`、`authoring_mainline_controller` 与 `grant_autonomy_controller` 缺少 `one-person-lab` owner、`codex_cli` default executor 或 OPL attempt lease/receipt 时必须返回 typed blocker，不能启动 MAG-owned durable loop。OPL 继续持有 hosted ledger、queue、workbench、scheduler/daemon、generic runner 和 executor substrate；MAG 不能外包 grant truth、memory body、fundability/quality/export verdict、submission-ready package authority、grant transition oracle 或 owner receipt。
...
真实 workspace/file lifecycle 由 OPL generic lifecycle primitive 与 MAG owner authority 分层完成。MAG caller 直接消费 `opl_harness_shared.workspace_boundary`、`schema_validation` 与 `artifact_lifecycle`；MAG repo source 只持有 declarative workspace/package profile、grant constraints、refs、policy、schema 和 authority proof。OPL 持有通用 locator/index、scheduler/runner/session/workbench shell、retention/restore orchestration 与 projection。真实 grant workspace、submission/export package、runtime artifact、receipt instance、cache、venv、pycache、pytest cache 和 install sync 副产物必须落在 workspace/runtime artifact root 或 `$CODEX_HOME/projects/med-autogrant/runtime-state/`，不能写回开发 checkout。
...
- `hermes_agent` / `claude_code` 是 OPL-owned 显式 opt-in backend；当前 MAG critique route 只声明/消费 `hermes_agent` 的 request / receipt vocabulary 与 OPL `AgentExecutionReceipt` 风格 proof，domain-handler dispatch 不会把它设为默认，也不会把 substrate implementation 写成 MAG-owned helper
...
这不等于“所有单步 authoring 都已经迁成 `hermes_agent` 非默认 executor lane”。
...
| Refs-only handoff / evidence | Hosted-receipt, lifecycle, memory, package-lifecycle, continuous-reconciliation, owner-payload and external-evidence surfaces. | MAG supplies domain refs、receipt validation、typed blockers、no-regression meaning and export authority；OPL shared owner-evidence/artifact-lifecycle primitives assemble generic reconciliation、observability and lifecycle envelopes. Neither side may infer grant readiness or expose body content from these refs. |
...
| Non-default executor receipt lane | `hermes_agent` / Claude Code remain explicit OPL Agent Executor Adapter backends; default executor remains `Codex CLI`. | MAG consumes only OPL request / receipt / closeout / critique payload vocabulary for explicit lanes. Connectivity or receipt proof does not imply provider reasoning quality, default runtime ownership, grant readiness, route migration or MAG-owned helper/runtime implementation. |
...
- 历史 Hermes substrate state/cache 只属于 OPL-owned 显式 proof lane 或 provenance；MAG 默认不要求安装上游 `hermes-agent`，也不管理 Hermes backend state。
