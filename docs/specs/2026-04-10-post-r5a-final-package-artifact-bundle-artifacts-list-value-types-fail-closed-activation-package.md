# Post-R5A Final Package Artifact Bundle Artifacts List Value Types Fail-Closed Activation Package
...
- Active slice: `build-final-package artifact bundle artifacts list value types fail-closed hardening`
- Status: `frozen / ready for implementation`
- Upstream prerequisite:
  - `R1.A / R1.B / R2.A / R3.A / R4.A / R5.A` 已 absorbed
  - post-`R5.A` walkthrough / output-consistency / worktree-aware hosted-contract / local-runtime validation-failed / final-package malformed artifact bundle / hosted-contract malformed final-package object-field / hosted-contract final-package required scalar-field / hosted-contract final-package required nested-field / hosted-contract final-package checkpoint-semantics / hosted-contract final-package lineage value-type / hosted-contract final-package freeze_manifest value-type / final-package artifact-bundle required nested-field / final-package artifact-bundle required scalar value-type / final-package artifact-bundle summary count value-type slices 已 absorbed
  - `build-final-package` 继续只属于本地 finalization / export boundary

## Goal

在不改写 `build-final-package` object boundary、不新增 runtime command、也不打开 actual hosted runtime / `P5` / 新 formal entry 的前提下，把当前 artifact bundle `artifacts.draft_outline / artifacts.draft_sections` list value-type 路径上的 fail-open gap 收紧成 repo-tracked fail-closed contract：

- 当 `artifacts.draft_outline / artifacts.draft_sections` 不是 `list` 时，`build-final-package` 必须稳定抛出 `WorkspaceStateError`
- 不得再出现：
  - `artifacts.draft_outline={}` 却仍成功产出 final package
  - `artifacts.draft_outline="oops"` 却仍成功产出 final package
  - `artifacts.draft_sections={}`、`artifacts.draft_sections=None`、`artifacts.draft_sections="oops"` 却仍成功产出 final package
- `build-final-package` 继续只消费 list value-type 正确的 artifact bundle；不放宽 `R2.A -> R4.A` 的输入 contract
...
- `_read_artifact_bundle(...)` 已开始校验：
  - `artifacts` object 存在
  - `artifacts.draft_outline / draft_sections` 字段存在
- 但仍未显式校验这两个 nested values 必须是 `list`

这导致多个 concrete gap：

1. `artifacts.draft_outline={}` 时，CLI 仍会成功产出 final package；
2. `artifacts.draft_outline="oops"` 时，CLI 仍会成功产出 final package；
3. `artifacts.draft_sections={}`、`artifacts.draft_sections=None`、`artifacts.draft_sections="oops"` 时，CLI 仍会成功产出 final package。

## In Scope

### 1. artifacts list value validation
...
- 不得继续成功产出 final package 掩盖 artifacts list drift
...
- 新 formal entry
- `artifacts.selected_direction / selected_question / argument_chain / fit_mapping` object value-type 校验
- `artifacts.draft_outline / draft_sections` 非空 list 语义
...
4. 构造非法 `artifacts.draft_outline / draft_sections` values，执行：
...
- `build-final-package` 继续只消费 `artifacts.draft_outline / draft_sections` 为 list 的 artifact bundle，不放宽 `R2.A -> R4.A` 输入 contract
- malformed artifact bundle 只能 fail-closed，不能 fallback、不能补默认值掩盖 list drift
