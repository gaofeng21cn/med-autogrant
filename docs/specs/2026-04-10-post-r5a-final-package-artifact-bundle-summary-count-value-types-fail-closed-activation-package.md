# Post-R5A Final Package Artifact Bundle Summary Count Value Types Fail-Closed Activation Package
...
- Active slice: `build-final-package artifact bundle summary count value types fail-closed hardening`
- Status: `frozen / ready for implementation`
- Upstream prerequisite:
  - `R1.A / R1.B / R2.A / R3.A / R4.A / R5.A` 已 absorbed
  - post-`R5.A` walkthrough / output-consistency / worktree-aware hosted-contract / local-runtime validation-failed / final-package malformed artifact bundle / hosted-contract malformed final-package object-field / hosted-contract final-package required scalar-field / hosted-contract final-package required nested-field / hosted-contract final-package checkpoint-semantics / hosted-contract final-package lineage value-type / hosted-contract final-package freeze_manifest value-type / final-package artifact-bundle required nested-field / final-package artifact-bundle required scalar value-type slices 已 absorbed
  - `build-final-package` 继续只属于本地 finalization / export boundary

## Goal

在不改写 `build-final-package` object boundary、不新增 runtime command、也不打开 actual hosted runtime / `P5` / 新 formal entry 的前提下，把当前 artifact bundle `bundle_summary` count value-type 路径上的 fail-open gap 收紧成 repo-tracked fail-closed contract：

- 当 `bundle_summary.outline_count / section_count` 不是**非负整数**时，`build-final-package` 必须稳定抛出 `WorkspaceStateError`
- 不得再出现：
  - `bundle_summary.outline_count="2"` 却仍成功产出 final package
  - `bundle_summary.outline_count=-1` 或 `bundle_summary.outline_count=False` 却仍成功产出 final package
  - `bundle_summary.section_count=None` 或 `bundle_summary.section_count=True` 却仍成功产出 final package
- `build-final-package` 继续只消费 count value-type 完整且类型正确的 artifact bundle；不放宽 `R2.A -> R4.A` 的输入 contract
...
- `_read_artifact_bundle(...)` 已开始校验：
  - `bundle_summary` object 存在
  - `bundle_summary.outline_count / section_count` 字段存在
- 但仍未显式校验这两个 count nested values 必须是非负整数

这导致多个 concrete gap：

1. `bundle_summary.outline_count="2"` 时，CLI 仍会成功产出 final package；
2. `bundle_summary.outline_count=-1`、`bundle_summary.outline_count=False` 时，CLI 仍会成功产出 final package；
3. `bundle_summary.section_count=None`、`bundle_summary.section_count=True` 时，CLI 仍会成功产出 final package。

## In Scope

### 1. artifact bundle summary count value validation
...
以上字段都必须是**非负整数**，且 `bool` 不得被当成合法 count。
...
- 不得继续成功产出 final package 掩盖 bundle summary count drift
...
4. 构造非法 `bundle_summary` count values，执行：
...
- `build-final-package` 继续只消费 `bundle_summary` count value-type 正确的 artifact bundle，不放宽 `R2.A -> R4.A` 输入 contract
- malformed artifact bundle 只能 fail-closed，不能 fallback、不能补默认值掩盖 count drift
