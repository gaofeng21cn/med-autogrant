...
当前仍由 docs guide 或 status 直接引用的 active current-truth specs 是：
...
更早的 dated `P*`、`R*`、`post-R5A`、activation-package、migration-board 与 tranche current-truth 文件属于历史技术记录。它们继续留在本目录作为 repo-tracked provenance，避免大范围重写旧审计路径；统一阅读入口改为：

- [历史 specs 索引](../history/specs/README.zh-CN.md)

当前产品真相不能只按文件名日期取最新 spec。请先读核心五件套和 `current-program.json`，再按需要读取 active specs 所冻结的具体边界。
