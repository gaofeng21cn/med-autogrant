...
其中 `build-final-package` 的 artifact-bundle 输入加载、output identity guard 与输出 handoff，以及 `build-hosted-contract-bundle` 的 final-package 输入加载、`program_id` control-plane 解析、identity guard 与输出 handoff，都已经切到 Hermes substrate owner path。
