# Post-R5A Final Package Artifact Bundle Artifacts Object Primary ID Fields Fail-Closed Activation Package
...
- 不得再出现：
  - `artifacts.selected_direction.direction_id=None` 或缺失 却仍成功产出 final package
  - `artifacts.selected_question.question_id=""` 或缺失 却仍成功产出 final package
  - `artifacts.argument_chain.argument_chain_id=None` 或缺失 却仍成功产出 final package
  - `artifacts.fit_mapping.fit_mapping_id=""` 或缺失 却仍成功产出 final package
- `build-final-package` 继续只消费 primary-id field 正确的 artifact bundle；不放宽 `R2.A -> R4.A` 的输入 contract
...
- 但仍未显式校验这些 object payload 的 primary-id fields

这导致多个 concrete gap：

1. `artifacts.selected_direction.direction_id` 缺失或为 `None` 时，CLI 仍会成功产出 final package；
2. `artifacts.selected_question.question_id` 缺失或为空字符串时，CLI 仍会成功产出 final package；
3. `artifacts.argument_chain.argument_chain_id` 缺失或为 `None` 时，CLI 仍会成功产出 final package；
4. `artifacts.fit_mapping.fit_mapping_id` 缺失或为空字符串时，CLI 仍会成功产出 final package。

## In Scope

### 1. object primary-id field validation
...
- 不得继续成功产出 final package 掩盖 object primary-id drift
...
4. 构造非法 object primary-id fields，执行：
...
- `build-final-package` 继续只消费上述 object primary-id fields 为非空字符串的 artifact bundle，不放宽 `R2.A -> R4.A` 输入 contract
- malformed artifact bundle 只能 fail-closed，不能 fallback、不能补默认值掩盖 object primary-id drift
