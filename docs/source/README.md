...
当前本目录只做 source/workspace 支撑索引；不复制 workspace truth、funder source state 或 source intake plan。机器边界读 `contracts/workspace_lifecycle_policy.json`、`contracts/runtime-program/opl-family-contract-adoption.json#source_provenance` 与 `product-entry-manifest.source_provenance`。新增内容必须保持真实 grant workspace、source body、memory body、artifact body 与 receipt instance 在 workspace/runtime artifact root 或用户级 runtime-state，repo source 只保存 schema、locator、policy、refs 和 no-forbidden-write 证据。
...
这些 refs 只服务 OPL locator/index/projection。MAG 继续持有 grant source truth、workspace truth、fundability / quality / export / submission verdict 和 receipt authority；通用 workspace/source intake shell、runtime、workbench、ledger 与 scheduler 仍归 OPL Framework / shared family layer。
