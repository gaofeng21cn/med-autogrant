...
### MAG stage output 接入 Stage-Native Artifact Contract

- 决策：MAG 每个 grant stage 必须在 `stage_native_artifact_contract` 中声明 required output role、manifest requirements、owner receipt / typed blocker requirements、current pointer 规则和 artifact classification boundary。`artifact_bundle` 只能作为某个 MAG stage output role 的 manifest/ref projection 存在，不能作为孤立 bundle truth。
...
- 影响：`contracts/stage_control_plane.json` 的每个 stage 都暴露 `stage_contract.stage_native_artifact_contract`；`src/med_autogrant/artifact_bundle.py` 生成的 payload 在 `stage_output_projection` 和 `manifest` 中投影 `stage_id`、`stage_output_role`、`artifact_classification`、`manifest_ref`、`current_pointer_ref` 与 `owner_receipt_or_typed_blocker_ref`。OPL 只能消费 refs、manifest 和 receipt/blocker；不得读取 artifact body、写 grant truth、推进 MAG current pointer、声明 fundability-ready、quality-ready、export-ready 或 submission-ready。
