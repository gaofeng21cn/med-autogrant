...
`package_and_submit_ready` 会在 Grant Meta Review 之后生成或转换四个 canonical package artifacts，并产生本地 submission-ready claim，因此不是纯 refs-only handoff。Producer 必须绑定同 generation 的四份 exact refs/hash；producer、repairer 和 repair-required reviewer/re-reviewer 只能返回 `route_impact.stage_route_recommendation`，terminal Reviewer/Re-reviewer 才返回 `route_impact.stage_route_decision`。Reviewer closeout 只提供审阅字段，正式 Review receipt 由 StageRunController 物化。本 Stage repair 只处理 assembly、manifest 和 provenance projection；上游内容、证据、质量闭环、附件 owner 或 export-verdict 缺陷必须 route-back。外部 portal acceptance 继续由下游 human owner 持有。
