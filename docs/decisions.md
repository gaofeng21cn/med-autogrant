...
- 决策：将 `contracts/external_evidence/mag-evidence-receipt-ledger.json` 从 request-level typed blocker accounting 推进为 `first_live_production_evidence_consumed_refs_only_long_soak_window_open`，7 条 external evidence request 都有 verified external receipt refs，并同步 production acceptance、executor-first program、current-program、OPL family adoption contract、status 与 active gap。
- 理由：MAG 需要能消费 external default caller、Codex App/workbench package refs、release/dist consumption、owner receipt / typed blocker roundtrip、continuous no-forbidden-write、direct/hosted parity no-regression 和 Temporal reconciliation refs 的首轮生产证据，同时继续只保存 refs、receipt shapes、typed blocker / no-regression refs 和 production acceptance refs。
- 影响：`remaining_real_evidence_gap_ids=["temporal_provider_long_soak_window_evidence"]` 表示 Temporal reconciliation ref 已记录，但长时 soak 窗口证据仍 open；它不授权 OPL/provider completion 替代 MAG 的 grant truth、fundability verdict、quality verdict、export verdict 或 submission-ready export，也不把 MAG 变成 OPL runtime、App workbench 或通用 production caller owner。

## 2026-05-21：默认运行口径收敛为 OPL/Temporal hosted autonomy

- 决策：`contracts/runtime-program/current-program.json`、product-entry runtime summary、hosted runtime substrate contract 与核心状态文档同步声明：任务启动后的默认运行 owner 是 OPL/Temporal hosted autonomous runtime；MAG 不实现 daemon、scheduler、attempt loop 或 attempt ledger；`Codex CLI` 只作为默认 stage executor。
- 理由：标准 OPL Agent 的 domain repo 应提交 declarative pack、domain handler、refs、typed blocker、owner receipt 和 grant authority surface。持久在线调度、唤醒、retry、resume、attempt ledger 和 long-running provider residency 是 OPL/Temporal 的职责，不能由 MAG 通过 product-entry、sidecar 或 autonomy controller 重新内置。
- 影响：direct app skill / CLI / `MedAutoGrantDomainEntry` 仍是一等入口，但运行驻留默认由 OPL/Temporal 承担；MAG 保持 grant truth、fundability / quality / export verdict、package authority、memory accept/reject 和 owner receipt authority。该决策不声明 Temporal long-soak window evidence 已关闭，也不授权 OPL 生成 grant-ready、fundability-ready、quality/export-ready 或 submission-ready verdict。
...
- 影响：该段已被同日 Lane 3 决策 supersede；当前机器真相是 7 条 request 都有 verified external receipt refs，`domain_owned_typed_blocker_count=0`，Temporal long-soak window evidence 仍 open。保留本段只作为中间状态 provenance。

## 2026-05-20：把 executor-first landing 收口成可用 read/guard surfaces

- 决策：新增 `codex_stage_execution_receipt_bundle`、`operator_closeout_readiness_projection` 与 `physical_morphology_guard_projection` 三个 product-entry surface，并统一暴露到 `MedAutoGrantProductEntry`。它们分别固定 Codex-first execution + independent review receipt ABI、operator closeout read model、physical source morphology role guard。
- 理由：AI-first / executor-first 路线需要让 Codex stage attempt、独立 review、operator closeout 和源码形态治理都可被 OPL/App/外部 caller 直接消费，同时不能把这些 read/guard surface 误升格成 MAG 私有 runtime 或 ready verdict generator。成熟 agent framework 的共同经验是把 executor/run、handoff/receipt、guardrail、state/checkpoint 和 workbench 分层；MAG 采纳分层模式，不引入外部 runtime 依赖。
- 影响：Codex stage execution 必须有 execution receipt 与独立 review receipt，缺 review 时 fail-closed；operator closeout 明确 request accounting closure、receipt coverage、real evidence 和 quality readiness 不等价；physical morphology guard 对 scheduler daemon、attempt ledger、local journal、generic runtime、App workbench、compatibility alias ownership fail-closed。该决策不声明 external caller/App/workbench 已消费 MAG，不声明 direct/hosted parity、continuous guard、Temporal long-soak 或 physical cleanup 完成，也不授权 fundability-ready / quality-ready / export-ready / submission-ready。
...
- 影响：该阶段的 sidecar dispatch action 后续已继续收薄；`status/read`、`user-loop/wakeup`、`notification/receipt`、`autonomy-controller/dry-run` 与 `autonomy-controller/guarded-run` 均不再作为 product sidecar dispatch action。autonomy-controller 的 domain authority 保留在 `MedAutoGrantDomainEntry` 与 CLI direct path；`hermes_agent` proof executor 仍是显式 opt-in，不成为默认 authoring executor。

- 2026-05-21 更新：当前 Codex-first / OPL typed queue 口径已收薄 `product sidecar dispatch`，`status/read`、`user-loop/wakeup`、`notification/receipt`、`autonomy-controller/dry-run` 与 `autonomy-controller/guarded-run` 这些 generic sidecar actions 退役。TODO/wakeup 仍可被 OPL queue 读取，但只输出 `open_grant_user_loop` / `product user-loop` action target、owner receipt / typed blocker / no-regression return shape 和 refs-only queue policy；dispatch 侧只保留 MAG receipt / refs-only domain actions。
...
- 决策：把 `Med Auto Grant` 的长线理想目标固定为 `OPL` stage-led framework、默认 `Codex CLI` concrete executor、显式 optional executor adapter/proof lane、`Med Auto Grant` domain truth owner 的分层结构，并在当时把主线阶段固定成 `P1 completed / P2 completed / P3 completed / P4 next`。
- 理由：如果不把 “理想目标” 和 “当前完成态” 分开冻结，后续最容易一边拿 `OPL` 理想型讲话，一边把未完成的 hosted caller / direct product entry 写成 landed。
- 影响：`current-program` 当时额外携带 `ideal_target` 与 `phase_map`；`P3 hosted caller / OPL consumption proof` 是 2026-04-12 的历史 proof context，当前不能用它声明外部 OPL/App/production caller、App/workbench consumption、bridge exit 或 long-soak 已完成。
