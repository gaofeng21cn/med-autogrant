...
- 2026-05-21 更新：当前 Codex-first / OPL typed queue 口径已收薄 `product sidecar dispatch`，`status/read`、`user-loop/wakeup` 与 `notification/receipt` 这三个 generic sidecar actions 退役。TODO/wakeup 仍可被 OPL queue 读取，但只输出 `open_grant_user_loop` / `product user-loop` action target、owner receipt / typed blocker / no-regression return shape 和 refs-only queue policy；dispatch 侧只保留 MAG domain handler guarded actions 与 closeout receipt actions。
