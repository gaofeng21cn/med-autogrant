...
- 影响：默认读根必须是 `current_owner_delta`；Capability Registry 只能是 Atlas/Pack/Stagecraft 的 catalog / ABI / use-policy，不是 grant authority；OPL/Vault/Console/Runway/Pack 不能签 MAG owner receipt、创建 MAG typed blocker 或授权 fundability / quality / export / submission verdict。该决策不声明 MAG production-ready 或 physical delete authority。
