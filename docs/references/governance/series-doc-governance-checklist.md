...
本清单用于把 `Med Auto Grant` 放进当前 OPL series 六仓治理口径里做巡检：`one-person-lab`、`one-person-lab-app`、`med-autoscience`、`med-autogrant`、`redcube-ai`、`opl-meta-agent`。
它服务跨仓 docs intake、回归与持续对齐，不替代 [MAG 文档组合治理](../../docs_portfolio_consolidation.md)、核心五件套、active plan、`docs/specs/README*` 列出的 active specs、`docs/specs/specs_lifecycle_map.md`、`schemas/v1/` 或 `contracts/runtime-program/`。
...
- 系列项目名称与角色要与当前六仓真相同步：`One Person Lab` 是完整的 stage-led、以 Agent executor 为最小执行单位 agent runtime framework；`One Person Lab App` 是 App/workbench 与 operator projection consumer；`Med Auto Science` 是 `Research Ops`；`Med Auto Grant` 是 `Grant Ops`；`RedCube AI` 是 visual-deliverable / `Presentation Ops`；`OPL Meta Agent` 是 Agent Foundry / new-agent builder-test managed module，不是 domain truth、artifact authority 或 owner receipt authority。
