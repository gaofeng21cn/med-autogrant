...
2026-05-20 状态：MAG 本仓的 migration/proposal/receipt surface、controlled consumed-memory proof、writeback receipt proof、runtime receipt evidence writer、operator receipt projection、OPL-hosted controlled grant stage attempt proof descriptor、标准 `family_domain_memory_ref.v1` adapter、`controlled_domain_memory_apply_proof` 和 repo-source layout audit 已落地。`domain_memory_descriptor` 是给 OPL family memory index 的薄引用面；`domain_memory_descriptor_locator` 仍是 MAG 侧 memory locator、migration plan、proposal、accept/reject、receipt 与 operator projection 的详细合同面。OPL 可把 MAG 解析为 resolved descriptor，MAG 也具备 accepted/rejected receipt evidence 写入路径；`contracts/external_evidence/mag-evidence-receipt-ledger.json` 已把 external request accounting 闭合为 receipt 或 typed blocker refs。但 external OPL/App/production caller consumption、owner receipt scaleout、direct/hosted parity 与 Temporal long soak 仍是真实 evidence gap。真实 memory body 仍由 workspace/runtime artifact root 产生，不进入 repo source；后续 OPL-hosted controlled grant stage soak 必须返回 MAG owner receipt、typed blocker 或 no-regression evidence。
