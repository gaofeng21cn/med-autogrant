...
- 根层 `functional_privatization_audit.mag_consumer_thinning_contract.active_path_scan_state` 必须来自真实 `physical_skeleton_follow_through.active_path_current_role_guard`，不能停留在 `not_available`。active source scan 只证明 current forbidden role or forbidden path 在 repo source 中没有复活；它不证明外部 production caller、App/workbench 消费或 Temporal long soak。
