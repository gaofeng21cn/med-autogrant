...
Codex plugin 安装和发现需要 plugin 目录下的真实 `SKILL.md` carrier。`plugins/med-autogrant/` 因此保留 `.codex-plugin/plugin.json` 与 `skills/med-autogrant/SKILL.md`，只解决安装、发现和 transport，不改变 MAG owner surface。
