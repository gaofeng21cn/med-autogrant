...
- 当前 family-shared substrate 复用真相：从 `2026-04-17` 起，`managed runtime contract` builder/validator，以及 `product_entry_quickstart / product_entry_overview / product_entry_readiness` 的公共 payload helper，都不再在 `Med Auto Grant` 内单独维护，而是直接复用 `one-person-lab` 持有的 `opl-harness-shared`；本仓只保留 grant-domain surface、schema、route truth、checkpoint truth 与 author-side 治理
