...
| Generated/default caller false-ready no-resurrection guard | `contracts/private_functional_surface_policy.json#/physical_source_morphology_policy/active_path_scan_policy` 会扫描 repo machine source 中把 `generated_surface_owner` 指回 MAG、允许 MAG 拥有 generated wrapper、或把 default-caller cutover / external caller consumption / OPL generated-hosted production caller / physical delete refs-only authorization 写成 true 的 JSON / Python / TOML / YAML 形态。 | 这是 strict source-purity 的 no-resurrection / false-ready guard；它只证明 repo-tracked machine source 不复活 MAG-owned generated surface claim，也不把 refs-only evidence 写成 default caller cutover complete、external caller consumption complete 或 physical delete authorized；不证明 generated/default caller parity、physical delete、App/operator consumption、grant readiness 或 Live Evidence closeout。 |
