...
- OPL owner-payload workorder 持续消费 `authority owner-payload-response` 输出，并在真实 stage/workspace target 上记录 owner receipt success refs path、stage expected receipt / monitor freshness / runtime event refs，或 domain-owned typed-blocker path；空模板、declared runtime refs 或 typed blocker refs 不得被读成 owner-chain/domain-ready/production-ready。
