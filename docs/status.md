...
| Gate | 当前状态 | 关闭条件 |
| --- | --- | --- |
| OPL-hosted grant-stage attempts | replay / monitor refs 可被 OPL 读取；owner-chain live progress evidence lane 已记录 MAG 在 OPL-hosted path 下可接受的 owner receipt、typed blocker、quality/export/package receipt 与 no-regression ref shapes。2026-06-09 canary 进一步证明 repo-native MAG authority commands 可对示例 workspace 返回 blocked quality / blocked package / body-free typed blocker / no-regression refs；持续 success-rate / long-run evidence 仍开放。 | 真实 OPL-hosted stage attempt 持续返回 MAG owner receipt、typed blocker、quality/export/package receipt 或 no-regression evidence，并形成可重复 success-rate / long-run evidence。 |
