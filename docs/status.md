...
| OPL StageRun consumption / grant-ready audit | `contracts/temporal_stage_run_consumption_policy.json` plus action catalog / generated surface handoff | MAG 只消费 OPL Temporal StageRun refs；provider completion、schema completeness、generated surface ready、manifest success、focused tests、stage replay、package existence 或 scorecard 分数不能声明 grant-ready、quality-ready、export-ready、submission-ready 或 production-ready。 |
...
| Private surface classification | `contracts/private_functional_surface_policy.json` and private inventory | Purpose-first thinning、source-ref integrity、source-purity、cleanup readback 和 no-resurrection guard 只证明边界、缺口和 fail-closed 状态；它们不授权 physical delete、default-caller cutover、App/operator sustained consumption、grant-ready 或 production-ready。 |
...
| Evidence / production / release lanes | evidence contracts and production acceptance refs | Temporal long-soak、submission human gate、App/operator sustained consumption、real workspace receipt scaleout、W7 closing ref、grant-ready 和 production-ready 是后置证据 lane；不再写成 status 或 active gap 的默认 worklist。 |
...
- Physical cleanup / no-resurrection 的关闭条件回到 active gap plan、private inventory 和 machine gates。No-active-caller evidence 只能证明 retired/tombstone surface 当前无 active caller，不授权删除仍有 active handler/adapter caller 的旧面；physical delete 仍需要 generated/default caller consumption、direct/hosted parity、owner receipt/typed blocker roundtrip、continuous no-forbidden-write 与 explicit MAG physical-delete owner receipt。
