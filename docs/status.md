...
2026-07-09 stage-size 审计读法：`contracts/stage_control_plane.json` 当前 6 个顶层 grant stage 保持不拆：`call_and_candidate_intake`、`fundability_strategy`、`specific_aims_and_structure`、`proposal_authoring`、`review_and_rebuttal`、`package_and_submit_ready`。它们已接近“一个 stage 一个主要开放判断”，分别对应 intake、fundability、aims structure、proposal authoring、review/rebuttal 和 package gate。OMA/OBF 的 stage 粒度问题不反向迁移到 MAG；stage prompt 只收口 route、refs、handoff、blocker，grant 方法层继续由 professional skills 承担。若某 route 在 stage 内反复失败，返回 route-local typed subpacket、repair target、typed blocker 或 route_back ref，不新增顶层 stage。
...
2026-07-10 shared primitive owner migration：MAG 已删除私有 `schema_subset_validator.py`、`workspace_scaffold.py` 和 `product_entry_parts/runtime_registration.py` 通用实现。workspace path/materialization/Git boundary、schema traversal、product preflight shape、runtime registration/family lifecycle envelope、receipt reconciliation/observability 和 package lifecycle refs-only envelope 现在由 `opl_harness_shared` 直接提供；MAG 只保留 declarative workspace/runtime/lifecycle profile、grant runtime constraints、owner receipt validation/signing、typed blocker/human gate、submission/export verdict normalization 和 package authority。该变更关闭这些模块的 source-level generic implementation residue，不等于 OPL hosted/default caller、App/workbench、Temporal long-soak、physical delete 或 grant/submission readiness 已完成。
...
| Standard Agent source shape | `contracts/functional_privatization_audit.json`, `contracts/runtime-program/opl-family-contract-adoption.json`, active gap plan | Workspace/schema、runtime registration、receipt reconciliation/observability 与 package lifecycle generic implementation 已切到 OPL shared primitive；`mag_functional_structure_gap_count=0` / `standard_agent_source_shape_status=landed` 仍只表示机器分类与这批 source owner 收口，不替代 product/status/user-loop/domain-handler/domain_runtime/autonomy/CLI shell 的 `physical-delete-not-authorized` tail。 |
