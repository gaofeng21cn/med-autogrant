...
- 当前 family-shared substrate 复用真相：从 `2026-04-17` 起，`managed runtime contract` builder/validator 不再在 `Med Auto Grant` 内单独维护，而是直接复用 `one-person-lab` 持有的 `opl-harness-shared`；本仓只保留 grant-domain surface、schema 与 author-side治理
