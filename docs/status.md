...
`contracts/live_stage_run_progress_evidence.json` 是当前 MAG-owned live stage progress owner answer / typed blocker source of truth。它把 OPL 标准消费路径需要的 `refs.owner_receipt_refs`、`refs.typed_blocker_refs`、`refs.human_gate_refs`、`refs.quality_or_export_receipt_refs`、`refs.no_regression_refs`、`refs.long_soak_refs`、`doc_refs` 与 `next_verification_command_refs` 固定为机器字段，并明确当前状态是 `blocked_by_mag_owned_typed_blocker`。`contracts/production_acceptance/mag-production-acceptance.json` 继续作为 production acceptance provenance，不作为 live stage progress owner answer，也不授权任何 ready claim。
