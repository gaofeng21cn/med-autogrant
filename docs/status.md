...
- 同一 manifest 现已把 domain memory 从 descriptor/locator 推进到可迁移/可验收的 repo-source contract：`domain_memory_descriptor_locator` 内含 migration plan、seed fixture ref 与 receipt locator，`contracts/runtime-program/domain-memory-seed-fixture.json` 只定义模板和禁止项，不包含真实 memory entry、workspace private evidence、grant artifact 或 receipt instance。已落地的是计划、模板和 locator；pending 的仍是 runtime writeback proposal generator、MAG accept/reject command 与 operator receipt projection。
