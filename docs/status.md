...
- OPL owner-payload workorder 持续消费 `authority owner-payload-response` 输出，并在真实 stage/workspace target 上记录 owner receipt success refs path、stage expected receipt / monitor freshness / runtime event refs，或 domain-owned typed-blocker path；默认 manifest consumer evidence 只证明 App/operator manifest 可以读取这些 refs 与 count-only scaleout provenance，不能替代实际 workorder payload record/verify。空模板、declared runtime refs 或 typed blocker refs 不得被读成 owner-chain/domain-ready/production-ready。
...
- OPL owner-payload refs：`authority receipt-readiness -> authority owner-payload-response -> OPL refs-only owner-payload record/verify`。
