...
2026-05-21 MAG 已在 `contracts/external_evidence/mag-evidence-receipt-ledger.json#/grant_stage_controlled_attempt_closeout/opl_stage_evidence_receipt_handoff` 声明 stage-level body-free owner receipt handoff refs。2026-05-22 OPL 已用这些 refs 关闭一条 `package_and_submit_ready` domain-dispatch workorder：`opl runtime action execute --action domain_dispatch:medautogrant:sat_da36b7ad2ca6609a6bfae2ba:record|verify` 记录并验证 `opl://external-evidence/medautogrant/domain_dispatch:medautogrant:sat_da36b7ad2ca6609a6bfae2ba`，payload 包含 `receipt:mag/grant-stage-controlled-attempt/package_and_submit_ready/owner-receipt-or-typed-blocker`、`typed-blocker:mag/package_and_submit_ready/submission_ready_export_gate/human-approval-required/2026-05-22`、`typed-blocker:mag/stage-source-runtime-live-evidence/package_and_submit_ready/pending`、owner-chain refs、direct/hosted parity no-regression ref 和 continuous no-forbidden-write ref。这个 closure 是 MAG-owned typed blocker / refs-only ledger 消费，不能把 `human_gate:submission_ready_export_gate` 当成 approval，也不能声明 grant-ready、fundability-ready、quality/export-ready、submission-ready、Temporal long-soak 或真实 App/workbench consumption 完成。

同日 MAG 进一步在同一个 closeout surface 下新增 `opl_stage_source_runtime_evidence_typed_blocker_handoff`，为 6 个 stage 的 source-scope / runtime-event live evidence tail 提供 MAG-owned typed blocker refs。该 handoff 覆盖 `contracts/stage_control_plane.json` 中声明的 29 个 source scope refs 和 6 个 runtime event refs；OPL 可以把这些 typed blocker refs 记录到 refs-only stage evidence ledger，从而把 workorder 明确关为 domain-owned blocker，而不是把声明型 source refs、runtime event 模板或 OPL workorder 本身写成成功证据。该状态不改变 7 条 external evidence request 的关闭统计，也不声明 source/runtime live evidence complete、grant-ready、fundability-ready、quality/export-ready、submission-ready 或 Temporal long-soak complete。
...
`mag_consumer_thinning_contract.generated_hosted_default_caller_proof` 现在作为 repo-local default-caller evidence gate 暴露到 product-entry manifest、product status 和 sidecar export。它把 MAG 当前 product/status/sidecar/grouped shell 分类为 repo-local handler/ref-only/migration input，要求 external evidence request 中的 `opl_generated_hosted_caller_consumption_evidence`、`direct_hosted_parity_no_regression_evidence` 与 continuous no-forbidden-write receipt 关闭后，才能声明 production/default caller 迁移完成。该 proof 只证明 MAG 侧 handler boundary 和 read-model proof 可验；它不声明 OPL generated/hosted production default caller 已完成 cutover、真实 App/workbench consumption 已完成或 Temporal long-soak 已完成。
...
Physical source morphology 当前按同一边界读取：`agent/` 是 Declarative Grant Pack，`contracts/` 是机器合同和 handoff，`src/med_autogrant/**` 中的 product-entry、sidecar、domain_runtime、runtime registration、lifecycle、package/memory projection、observability 或 workbench/scheduler metadata 只能是 direct domain handler、refs-only adapter、minimal authority function、diagnostic 或 history/tombstone。MAG 不再维护 local journal、attempt ledger、repo-owned scheduler daemon、Hermes/Gateway/local-manager probe 或 compatibility facade；后续新增功能默认进入 `agent/` pack、`contracts/`、authority function 或 domain handler，不恢复本地 runtime 平台。当前机器 policy 已把 grouped CLI wrapper 与 owner receipt helper 纳入 physical morphology 分类，并要求 active caller migration、direct/hosted parity、owner receipt / typed blocker roundtrip、continuous no-forbidden-write 与 no-active compatibility alias scan 同时成立后，才允许删除、改名或 tombstone。

当前物理源码仍不是“新建标准 Agent 模板”形态：`domain_runtime_parts/substrate.py`、`domain_entry.py`、`cli.py`、`product_entry_parts/manifest_builder.py` / `manifest_builder_parts/shell_assembly.py` / `manifest.py` / `sidecar.py` / `runtime_registration.py`、`control_plane.py`、owner receipt helper、`grant_autonomy_controller.py` 和 `grant_autonomy_loop_shell.py` 仍是 active direct path、refs-only adapter、authority refs、regression oracle 或 migration input。2026-05-21 `sidecar.py` 已继续把 generic export projection helpers 拆入 `sidecar_projection.py`、把 shared constants 拆入 `sidecar_contract.py`，并把 task dispatch / receipt wrapping 拆入 `sidecar_dispatch.py`；主 `sidecar.py` 降到 312 行，只保留 active sidecar export builder 与公开 dispatch re-export。同日 `manifest_builder.py` 已把 generic product-entry shell / operator loop / family action-stage manifest assembly 拆入 `manifest_builder_parts/shell_assembly.py`，主 builder 降到 724 行并继续保留 MAG direct manifest 编排、workspace/route 读取、domain memory、receipt、quality/export verdict refs 与 owner authority surfaces。2026-05-22 `grant_autonomy_controller.py` 已从 961 行降到 171 行，只保留 request/start/governance 编排；loop/scheduler-like control envelope 迁入 `grant_autonomy_loop_shell.py`，generic report/history envelope 进入 `grant_autonomy_loop_parts.py`。最新收薄已删除 883 行的 `grant_autonomy_parts.py` 聚合文件，并把 active caller 直接改为消费 `grant_autonomy_common.py`、`grant_autonomy_controller_plan.py`、`grant_autonomy_quality_payload.py` 与 `grant_autonomy_report_resume.py`；其中 report/resume envelope 是 OPL attempt lifecycle / generated operator loop 的迁移输入，controller plan 与 quality payload 仍是 MAG grant authority support。这个拆分不是 MAG-owned generic runtime，也不是 OPL 已接管 autonomy controller。这些只是收薄 direct manifest/sidecar/controller adapters，不表示 OPL generated product-entry/status/workbench/sidecar/controller 已 default 化。上述 surfaces 不能被写成 MAG-owned generic runtime，也不能被写成已经物理清零。后续 cleanup 需要 OPL generated caller / App / production default caller evidence、direct/hosted parity、owner receipt roundtrip、no-forbidden-write 和 active caller migration 同时成立后再改名、迁出、删除或 tombstone。本轮只做有证据的退役与文档归档；这些 retained surfaces 继续作为 explicit remainder 保留。
