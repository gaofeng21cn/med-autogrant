...
The current entry truth is also explicit: `operator entry` and `agent entry` are real today, and the lightweight grant `product entry` shell is now landed. `product-entry-manifest` is now the machine-readable discovery surface for that shell, `product-frontdesk` now freezes the controller-owned direct front door above it, and the shared envelope plus routing surfaces are frozen as schema-backed contract surfaces. The first honest `P4.A` direct-product projection is now also landed through `grant-progress` and `grant-cockpit`, the next honest `P4.B` direct-entry composition is now landed through `grant-direct-entry`, the current `P4.C` companion layer is now landed through `mainline-status`, `mainline-phase`, and `grant-user-loop`, and `P4.D` now lands the full authoring executor ladder from `direction_screening` to `frozen`; all of these product-facing and route-facing surfaces are repo-tracked and fail-closed, while a richer grant-facing product experience still remains future work.
...
- Product-entry discovery surface: `product-entry-manifest` now freezes the current grant shell, shared handoff templates, and current mainline snapshot into one machine-readable manifest for direct callers and `OPL`.
...
- Schema-backed contract closeout: the landed `product entry` shell, `executor_routing_contract`, the full authoring command surface, `grant-progress`, `grant-cockpit`, `grant-direct-entry`, `grant-user-loop`, and service-safe route surfaces are now indexed under `schemas/v1/` and validated fail-closed at generation time.
