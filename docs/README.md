...
Fundability, specific aims, reviewer grammar, and template strategy experience is managed as natural-language memory. `domain_memory_descriptor_locator`, `controlled_domain_memory_apply_proof`, `owner_receipt_contract`, and `lifecycle_guarded_apply_proof` project writeback proposal / accept-reject / owner / no-regression / lifecycle runtime receipt evidence refs and repo-source layout audit only; they do not write real memory entries, grant artifacts, or export verdicts into repo source. `workspace quality-scorecard`, `grant-quality-closure-dossier`, autonomy-controller reports, and submission-ready packages remain structured authority.
