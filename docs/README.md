...
- MAG now exposes controlled grant-stage domain memory apply proof through `controlled_domain_memory_apply_proof`: consumed grant strategy memory refs, writeback proposal, MAG accept/reject decision, runtime receipt evidence, operator receipt projection, and repo-source layout audit are verifiable without storing memory body, grant artifacts, or receipt instances in repo source.
