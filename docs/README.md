...
`plugins/med-autogrant/.codex-plugin/plugin.json` 与 skill source；Codex config 的 marketplace
由 OPL-owned wrapper 生成，不再把 `.agents/plugins/marketplace.json` 写回本仓。
同一测试也继续确认 tracked source/test line budget 作为单一 hard gate 生效；
新增或增长的超长文件应拆分，而不是扩大单文件基线或恢复 advisory/strict 双轨语义。
...
- 文档生命周期治理规则统一留在 [MAG 文档组合治理](./docs_portfolio_consolidation.md)。[系列项目文档治理清单](./references/governance/series-doc-governance-checklist.md) 只作为 OPL series 跨仓 docs intake / 对齐巡检支撑，不替代本文档索引、文档组合治理、核心五件套、active plan、specs lifecycle map 或 machine contracts。
