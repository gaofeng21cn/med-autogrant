...
The current entry truth is also explicit: `operator entry` and `agent entry` are real today, and the lightweight grant `product entry` shell is now landed. Its shared envelope and routing surfaces are also frozen as schema-backed contract surfaces. The first honest `P4.A` direct-product projection is now also landed through `grant-progress` and `grant-cockpit`, and the next honest `P4.B` direct-entry composition is now landed through `grant-direct-entry`; all three surfaces are schema-backed and generation-time fail-closed under `schemas/v1/`, while a richer grant-facing product experience still remains future work.
...
- Schema-backed contract closeout: the landed `product entry` shell, `executor_routing_contract`, `pending_handoff_requirements`, `grant-progress`, `grant-cockpit`, and service-safe route surfaces are now indexed under `schemas/v1/` and validated fail-closed at generation time.
