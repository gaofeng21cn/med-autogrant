...
The current entry truth is also explicit: `operator entry` and `agent entry` are real today, and the lightweight grant `product entry` shell is now landed. Its shared envelope and routing surfaces are also frozen as schema-backed contract surfaces. The first honest `P4.A` direct-product projection is now also landed through `grant-progress` and `grant-cockpit`, and these two projections are now also schema-backed and generation-time fail-closed under `schemas/v1/`, while a richer grant-facing product experience still remains future work.
