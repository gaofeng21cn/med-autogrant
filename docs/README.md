...
The current entry truth is also explicit: `operator entry` and `agent entry` are real today, and the lightweight grant `product entry` shell is now landed. Its shared envelope and routing surfaces are also frozen as schema-backed contract surfaces. The first honest `P4.A` direct-product projection is now also landed through `grant-progress` and `grant-cockpit`, the next honest `P4.B` direct-entry composition is now landed through `grant-direct-entry`, and the current `P4.C` companion layer is now landed through `mainline-status`, `mainline-phase`, and `grant-user-loop`; all of these product-facing surfaces are repo-tracked and fail-closed, while a richer grant-facing product experience still remains future work.
