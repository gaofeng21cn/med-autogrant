...
- Product frontdesk surface: `product-frontdesk` now freezes the controller-owned direct front door above the grant shell into one machine-readable frontdesk contract, including a `grant_authoring_readiness` maturity companion, and it is now fail-closed by `product-frontdesk.schema.json`.
