...
- 检查 OPL Foundry Agent series command surface 时，用 `uv run mag foundry status --json` 或 `<med-autogrant-repo>/scripts/run-python-clean.sh -m med_autogrant.cli foundry status --format json`；`mag` 与 `medautogrant` 等价。help 首屏会显示 `Series: OPL Foundry Agent`、`Agent id: medautogrant`、ordinary path `workspace/work/stage/run/vault/handoff/connect`、executable command surface 和 MAG authority boundary。`status`、`inspect`、`interfaces`、`validate`、`doctor` 和 `peers` 是顶层快捷命令，`grant` alias 到 `workspace`，`work` alias 到 `pass`。
