...
- MAG 继续独立持有 grant truth、fundability verdict、authoring quality verdict、route owner 与 submission/export authority；`Hermes-Agent` 与旧 `OPL Runtime Manager` wording 只属于显式 provider / proof / migration / provenance 语境。
...
- `contracts/runtime-program/current-program.json` 现在还额外固定了 `stage_led_framework_boundary`、`ideal_target` 与 `phase_map`，用于说明 `Med Auto Grant` 独立 domain agent、`OPL` Codex-first stage-led framework、显式 hosted/proof carrier（如外部 `Hermes-Agent`）的技术位置，以及 `OPL` family-level 协作边界与当前阶段。
