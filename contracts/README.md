...
- 当前 machine-readable 入口分层固定为：`CLI` / `MedAutoGrantDomainEntry`（agent entry）与 `product entry/frontdesk/direct-entry/user-loop`（lightweight direct entry / projection）。
...
- `contracts/runtime-program/current-program.json` 现在还额外固定了 `ideal_target` 与 `phase_map`，用于说明 `Med Auto Grant` 独立 domain agent、`Hermes-Agent` runtime substrate owner、以及 `OPL` family-level 协作边界与当前阶段。
